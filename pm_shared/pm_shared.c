...
*times when you use a ladder, duck and so on
+texture types
...
//new
#define CHAR_TEX_BLUE		'B'
#define CHAR_TEX_YELLOW		'X'
#define CHAR_TEX_BLACK		'Z'
#define CHAR_TEX_SNOW		'N'//this make a new sound
#define CHAR_TEX_RED		'R'
#define CHAR_TEX_SLIME		'U'
...
void PM_PlayStepSound( int step, float fvol )
{
	...
	int irand;
	vec3_t hvel;
//	qboolean canmakeglasssounds = false;//sys
	...
	// See if user can make glass sounds?
//	canmakeglasssounds = atoi( pmove->PM_Info_ValueForKey( pmove->physinfo, "glass" ) ) == 1 ? true : false;
	...
	//agregar igual?
	/*
	if (canmakeglasssounds)
	{
		switch (irand)
		{
		// right foot
		case 0:	pmove->PM_PlaySound( CHAN_BODY, "player/pl_glass1.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 1:	pmove->PM_PlaySound( CHAN_BODY, "player/pl_glass2.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		// left foot
		case 2:	pmove->PM_PlaySound( CHAN_BODY, "player/pl_glass2.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 3:	pmove->PM_PlaySound( CHAN_BODY, "player/pl_glass1.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		}
	}
	*/
}	
...
//		usingmoto = atoi( pmove->PM_Info_ValueForKey( pmove->physinfo, "moto" ) ) == 1 ? true : false;
...
void PM_Jump (void)
{
    ...
    pmove->onground = -1;
// Referencias:
// [ 2 ] = -15 (circular derecha)
// [ 2 ] = 15 (circular izquierda)
// [ 0 ] = -15 (eje Y abajo)
// [ 0 ] = 15 (eje Y arriba)
	switch ( pmove->RandomLong(0,3) )
	{
		case 0: pmove->punchangle[ 2 ] = -8; //10
				pmove->punchangle[ 0 ] = -8; 
				pmove->PM_PlaySound( CHAN_BODY, "player/jump01.wav", 0.9, ATTN_NORM, 0, PITCH_NORM );
		break; 

		case 1: pmove->punchangle[ 2 ] = 6; 
				pmove->punchangle[ 0 ] = 6; 
				pmove->PM_PlaySound( CHAN_BODY, "player/jump02.wav", 0.9, ATTN_NORM, 0, PITCH_NORM );
		break; 

	 	case 2: pmove->punchangle[ 2 ] = -4; 
				pmove->punchangle[ 0 ] = -4; 
				pmove->PM_PlaySound( CHAN_BODY, "player/jump03.wav", 0.9, ATTN_NORM, 0, PITCH_NORM );
		break; 
	
		case 3: pmove->punchangle[ 2 ] = 3; 
				pmove->punchangle[ 0 ] = 3; 
				pmove->PM_PlaySound( CHAN_BODY, "player/jump04.wav", 0.9, ATTN_NORM, 0, PITCH_NORM );
		break;
	}
...
*/
		...
		{ 
			...
			if (pmove->chtexturetype == CHAR_TEX_CONCRETE ) 
			pmove->PM_PlaySound( CHAN_BODY, "player/pl_step1.wav", 0.9, ATTN_NORM, 0, PITCH_NORM ); 
			
			else if 
			(pmove->chtexturetype == CHAR_TEX_METAL ) 
			pmove->PM_PlaySound( CHAN_BODY, "player/pl_metal1.wav", 0.9, ATTN_NORM, 0, PITCH_NORM ); 
		
			else if 
			(pmove->chtexturetype == CHAR_TEX_DIRT ) 
			pmove->PM_PlaySound( CHAN_BODY, "player/pl_dirt1.wav", 0.9, ATTN_NORM, 0, PITCH_NORM ); 
		
			else 
			pmove->PM_PlaySound( CHAN_BODY, "player/pl_step1.wav", 0.9, ATTN_NORM, 0, PITCH_NORM ); 

			pmove->punchangle[ 0 ] = pmove->flFallVelocity * 0.013; // punch x axis 
