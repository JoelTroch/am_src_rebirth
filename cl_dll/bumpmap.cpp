...
//DONE: Thanks to protector for give me his XTREE file :) (the mine was damaged or something)
...
//sys: we will use the glow's def
...
bool CBumpmapMgr::Initialise(void)
{
	...
	gEngfuncs.pfnRegisterVariable("bm_enable", "1", 0);
	...
	while (fgets(szTex, 256, pFile))
	{
		...
		*pBumpFile = '\0';
		...
		for (int i = 0; i < pWorld->numtexinfo; i++)
		{
			...
			{
				sprintf(szTexPath, "%s/shaders/bumpmap/%s", gEngfuncs.pfnGetGameDirectory(), pBumpFile);
...
{
	// fullbright map
	if (!LoadCgProgram(&m_fp_Diffuse, m_cg_fragProfile, "shaders/fp_diffuse.cg"))
	{
	...
	if (!LoadCgProgram(&m_fp_Lightmaps, m_cg_fragProfile, "shaders/fp_lightmaps.cg"))
	{
	...
	// bumpmapping
	if (!LoadCgProgram(&m_fp_Bump, m_cg_fragProfile, "shaders/fp_bump.cg"))
	{
	...
	// fullbright model
	if (!LoadCgProgram(&m_fp_ModelPass0, m_cg_fragProfile, "shaders/fp_model_p0.cg"))
	{
	...
	if (!LoadCgProgram(&m_fp_ModelPass1, m_cg_fragProfile, "shaders/fp_model_p1.cg"))
	{
...
void CBumpmapMgr::AddLight(const char* targetname, Vector pos, Vector colour, float strength, float radius,
						   bool enabled , int style, int moveWithEntID , bool moveWithExtraInfo ,
