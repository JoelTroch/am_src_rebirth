...
CM16	     g_M16;//this is M16

CHL2Pistol g_Elite;

CHeal			 g_Heal;

CHL2Shotgun	g_HL2Shotgun;
CHL2Crowbar	g_HL2Crowbar;

CHL2HandGrenade	g_HL2HandGrenade;
...
//	HUD_PrepEntity( &g_Satchel	, &player );
	...
	HUD_PrepEntity( &g_M16			, &player );//m16

	HUD_PrepEntity( &g_Elite	, &player );
	HUD_PrepEntity( &g_Crossbow	, &player );

	HUD_PrepEntity( &g_HL2Crowbar		, &player );
	HUD_PrepEntity( &g_HL2Shotgun		, &player );
	HUD_PrepEntity( &g_HL2HandGrenade	, &player );
...
void HUD_WeaponsPostThink( local_state_s *from, local_state_s *to, usercmd_t *cmd, double time, unsigned int random_seed )
{
	...
	{		
		case WEAPON_HL_MEDKIT:        
			pWeapon = &g_Heal;        
			break;

		case WEAPON_HL_GAUSS:        
			pWeapon = &g_Gauss;        
			break;
		...
		case WEAPON_HL_DEAGLE:        
			pWeapon = &g_Eagle;        
			break;

		case WEAPON_HL_SNIPER:        
			pWeapon = &g_Sniper;        
			break;
		...
		case WEAPON_HL_GLOCK:
		  pWeapon = &g_Elite;
		  break;

		 case WEAPON_HL_SHOTGUN:
		  pWeapon = &g_HL2Shotgun;
		  break;

		case WEAPON_HL_CROWBAR:
		  pWeapon = &g_HL2Crowbar;
		  break;

		case WEAPON_HL_HANDGRENADE:
		  pWeapon = &g_HL2HandGrenade;
		  break;

		case WEAPON_HL_PYTHON:
			pWeapon = &g_Python;
			break;
...
//			break;
		...
		case WEAPON_HL_CROSSBOW:
			pWeapon = &g_Crossbow;
			break;

		case WEAPON_HL_HORNETGUN:
			pWeapon = &g_HGun;
			break;

//		case WEAPON_HL_SATCHEL:
//			pWeapon = &g_Satchel;
//			break;

		case WEAPON_HL_TRIPMINE:
			pWeapon = &g_Tripmine;
			break;

		case WEAPON_HL_SNARK:
			pWeapon = &g_Snark;
			break;
	}
...
//		pCurrent->m_flPumpTime			= pfrom->m_flPumpTime;
		...
		//SP
		pCurrent->m_fDefaultAnim		= pfrom->iuser3; //Hier wir der Wert aus dem Server übernommen
	...
	if ( player.m_pActiveItem->m_iId == WEAPON_HL_RPG )
	{
...
//		pto->m_flPumpTime				= pCurrent->m_flPumpTime;
		...
		pto->iuser3						= pCurrent->m_fDefaultAnim; //SP
