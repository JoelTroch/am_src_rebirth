...
void DLLEXPORT CAM_Think( void )
{
	...
	if ( player && (gEngfuncs.GetMaxClients() > 0) && (gViewPort && gEngfuncs.pEventAPI) && frames >= 5 )
	{
		...
		gEngfuncs.pEventAPI->EV_PlayerTrace( origin, origin - (camForward * (c_maxdistance->value + 16)), PM_STUDIO_BOX, -1, &tr );

		if(tr.fraction < 1.0 )
		{
			dist = c_maxdistance->value * tr.fraction;
...
*/
	...
	if ( gEngfuncs.GetMaxClients() == 1 )
	{
		// no thirdperson in singleplayer.
		return;
	}
*/
	gEngfuncs.GetViewAngles( (float *)viewangles );
