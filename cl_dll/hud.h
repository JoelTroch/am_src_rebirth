...
//jeje, hide hud when it's not needed
#define MIN_ALPHA	 50
//#define MIN_ALPHA	 999	
...
class CHudLuzBar: public CHudBase
{
public:
	int Init( void );
	int VidInit( void );
	int Draw(float flTime);
	void Reset( void );
	int MsgFunc_LuzBar(const char *pszName,  int iSize, void *pbuf );
...
private:
	...
	HSPRITE m_hLuzBack;
	HSPRITE m_hHealthArmor;
	HSPRITE m_hHudTalk;
	HSPRITE m_hHudHandWeapon;
	HSPRITE m_hHudHand;
};
...
//*******************************************
class CHudCine: public CHudBase
{
public:
int Init( void );
int VidInit( void );
...
void Reset( void );
int MsgFunc_CineBars(const char *pszName, int iSize, void *pbuf);

private:
HSPRITE m_hFlickerCine;
int m_iOn;
};

//*******************************************
class CHudRicardo: public CHudBase
{
public:
int Init( void );
int VidInit( void );
...
void Reset( void );
int MsgFunc_RicardoBars(const char *pszName, int iSize, void *pbuf);

private:
HSPRITE m_hFlickerRicardo;
int m_iOn;
};
...
//**************
class CHudFlashBar: public CHudBase
{
public:
	int Init( void );
	int VidInit( void );
	int Draw(float flTime);
	void Reset( void );
	int MsgFunc_FlashBar(const char *pszName,  int iSize, void *pbuf );
	
private:
	HSPRITE m_hSprite1;
	HSPRITE m_hSprite2;
	HSPRITE m_hLuzBarBack;
...
//	int							m_iConcussionEffect; //SP moved to public
...
//	~CHud();			// destructor, frees allocated memory
	...
	int  _cdecl MsgFunc_Clcommand	( const char *pszName, int iSize, void *pbuf );
...
//    void _cdecl MsgFunc_FPlaySound( const char *pszName, int iSize, void *pbuf );    		// fo0z
