...
TeamFortressViewport::TeamFortressViewport(int x,int y,int wide,int tall) : Panel(x,y,wide,tall), m_SchemeManager(wide,tall)
{
	...
	m_pTeamMenu = NULL;
	m_pClassMenu = NULL;
	m_pScopePanel = NULL; //test
	m_pScoreBoard = NULL;
	m_pSpectatorPanel = NULL;
	m_pCurrentMenu = NULL;
	m_pCurrentCommandMenu = NULL;
	m_pCinePanel = NULL; //test
	m_pRicardoPanel = NULL; //test
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
		m_StandardMenu = CreateCommandMenu("commandmenu_mp.txt", 0, CMENU_TOP, false, CMENU_SIZE_X, BUTTON_SIZE_Y, 0 );
	else
...
{
	 if (m_pScopePanel)//test
	   {
		  m_pScopePanel->setVisible( false );
	   }

	 if (m_pCinePanel)//test
	  {
		  m_pCinePanel->setVisible( false );
	  }

	 if (m_pRicardoPanel)//test
	  {
		  m_pRicardoPanel->setVisible( false );
	  }
...
/////////////
void TeamFortressViewport::ShowCine()
{
   if (m_pCinePanel)
   {
      //m_pScopePanel->Open();
      m_pCinePanel->setVisible( true );
   }
}

void TeamFortressViewport::HideCine()
{
   if (m_pCinePanel)
   {
      m_pCinePanel->setVisible( false );
...
void TeamFortressViewport::CreateCine()
{
   // Create the panel
   m_pCinePanel = new CCinePanel(0, 0, ScreenWidth, ScreenHeight);
   m_pCinePanel->setParent(this);
   m_pCinePanel->setVisible( false );
   m_pCinePanel->Update();
}
/////////////
void TeamFortressViewport::ShowRicardo()
{
   if (m_pRicardoPanel)
   {
      //m_pScopePanel->Open();
      m_pRicardoPanel->setVisible( true );
   }
}

void TeamFortressViewport::HideRicardo()
{
   if (m_pRicardoPanel)
   {
      m_pRicardoPanel->setVisible( false );
...
void TeamFortressViewport::CreateRicardo()
{
   // Create the panel
   m_pRicardoPanel = new CRicardoPanel(0, 0, ScreenWidth, ScreenHeight);
   m_pRicardoPanel->setParent(this);
   m_pRicardoPanel->setVisible( false );
   m_pRicardoPanel->Update();
}
//======================================================================================
//======================================================================================
// SPECTATOR MENU
