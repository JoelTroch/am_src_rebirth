...
*/
		...
		if (pplayer->weaponmodel)
		{
			...
			//SysOp: This is the only way I can prevent to draw the p_ model
			//this is for player's body (in the first test I saw the p_ model in first person)
			//the GetPlayer func, draws the entire player, but i just need only the body
...
//	int iShowPlayer = CVAR_GET_FLOAT("cl_showplayer");
	...
	else
	{
		for (i=0 ; i < m_pStudioHeader->numbodyparts ; i++)
		{
			...
			//raMp's Fix		
			if ( iShadows == 2 )//uses the original code
			{
				GL_StudioDrawShadow = (void(*)(void))(((unsigned int)IEngineStudio.GL_StudioDrawShadow)+32);
				HackShadows();
		
			}		
			if ( iShadows == 1 )//draw the shadows only on certain entities and monsters
			{		
				//edit: check the render fx first
				if (m_pCurrentEntity->curstate.renderfx != 255)//if the entity has the renderfx Nº 255, then don't draw shadows
				{
					if (m_pCurrentEntity->curstate.movetype == MOVETYPE_STEP) 
					{
						GL_StudioDrawShadow = (void(*)(void))(((unsigned int)IEngineStudio.GL_StudioDrawShadow)+32);
