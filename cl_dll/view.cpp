...
// get frame time
	//BUzer's CODE
	//TO DO:
	//I can't make it work. This is used to simulate a MoveIn mod hand movement style (he (or she) said that).
/*
	static float newlasttime = 0;// 0
	...
	static vec3_t gunAngles; // current gun angles
	for (i = 0; i < 3; i++)///for (int i = 0; i < 3; i++)
	{
		// 10 is some speed factor, tweak it
	   float speed = (gunAngles[i] - pparams->viewangles[i]) * 10;
		if (speed < 0) speed *= -1;

		if (gunAngles[i] < pparams->viewangles[i])
		{
			gunAngles[i] += frametime * speed;
			if (gunAngles[i] > pparams->viewangles[i])
				gunAngles[i] = pparams->viewangles[i];
		}
		else if (gunAngles[i] > pparams->viewangles[i])
		{
			gunAngles[i] -= frametime * speed;
			if (gunAngles[i] < pparams->viewangles[i])
				gunAngles[i] = pparams->viewangles[i];
