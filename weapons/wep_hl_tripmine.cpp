...
{
	PRECACHE_MODEL("models/weapons/tripmine/v_tripmine.mdl");
...
{
	PRECACHE_MODEL ("models/weapons/tripmine/v_tripmine.mdl");
	PRECACHE_MODEL ("models/weapons/tripmine/p_tripmine.mdl");
	UTIL_PrecacheOther( "monster_tripmine" );

	m_usTripFire = PRECACHE_EVENT( 1, "scripts/events/tripfire.sc" );
}
...
int CTripmine::GetItemInfo(ItemInfo *p)
{
	...
	p->iSlot = 4;
	p->iPosition = 2;
	p->iId = m_iId = WEAPON_HL_TRIPMINE;
	p->iWeight = 10;
...
{
	//pev->body = 0;
	return DefaultDeploy( "models/weapons/tripmine/v_tripmine.mdl", "models/weapons/tripmine/p_tripmine.mdl", TRIPMINE_DRAW, "trip" );
}
...
void CTripmine::Holster( int skiplocal /* = 0 */ )
{
	...
	{
		// out of mines
		m_pPlayer->pev->weapons &= ~(1<<WEAPON_HL_TRIPMINE);
...
int CTripmine::AddToPlayer( CBasePlayer *pPlayer )
{
    ...
    {
		ClientPrint(m_pPlayer->pev, HUD_PRINTCENTER, "#Pickup_Tripmine"); //digamos al cliente

		m_pPlayer->SetSuitUpdate("!HEV_TRIPMINE", FALSE, SUIT_NEXT_IN_5SEC);
