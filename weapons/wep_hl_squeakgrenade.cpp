...
{
	PRECACHE_MODEL("models/weapons/squeak/w_squeak.mdl");
...
{
	Precache( );
	m_iId = WEAPON_HL_SNARK;
	SET_MODEL(ENT(pev), "models/weapons/squeak/w_sqknest.mdl");
...
{
	PRECACHE_MODEL("models/weapons/squeak/w_sqknest.mdl");
	PRECACHE_MODEL("models/weapons/squeak/v_squeak.mdl");
	PRECACHE_MODEL("models/weapons/squeak/p_squeak.mdl");
...
BOOL CSqueak::Deploy( )
{
	...
	return DefaultDeploy( "models/weapons/squeak/v_squeak.mdl", "models/weapons/squeak/p_squeak.mdl", SQUEAK_UP, "squeak" );
}
...
void CSqueak::Holster( int skiplocal /* = 0 */ )
{
	...
	{
		m_pPlayer->pev->weapons &= ~(1<<WEAPON_HL_SNARK);
...
int CSqueak::AddToPlayer( CBasePlayer *pPlayer )
{
    ...
    {
		ClientPrint(m_pPlayer->pev, HUD_PRINTCENTER, "#Pickup_Squeak"); //digamos al cliente

		m_pPlayer->SetSuitUpdate("!HEV_HGUN", FALSE, SUIT_NEXT_IN_5SEC);
