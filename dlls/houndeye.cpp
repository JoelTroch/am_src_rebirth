...
#define HOUNDEYE_LEFTLEG			6//6
#define HOUNDEYE_RIGHTLEG			7//7
#define HOUNDEYE_MIDDLELEG			8
...
#define LEG_LEFT_GROUP				2
#define LEG_LEFT_NONE				1

#define LEG_MIDDLE_GROUP			3
#define LEG_MIDDLE_NONE				1
...
{	
	...
	PRECACHE_MODEL("models/houndeye_gibs.mdl");
...
void CHoundeye :: DeathSound ( void )
{
	...
	{
		SetBodygroup( EYE_GROUP, EYE_NONE );
...
//		CGib::SpawnStickyGibs(pev, pev->origin ,8);
		...
		ALERT ( at_console, ">Cortando eye \n" );
	}
	else if (m_LastHitGroup == HOUNDEYE_LEFTLEG)
	{
		SetBodygroup( LEG_LEFT_GROUP, LEG_LEFT_NONE );
		CGib::HoundeyeGibs( pev, 1, 0 ); 
		ALERT ( at_console, ">Cortando LEG_LEFT \n" );
	}
	else if (m_LastHitGroup == HOUNDEYE_MIDDLELEG)
	{
		SetBodygroup( LEG_MIDDLE_GROUP, LEG_MIDDLE_NONE );
		CGib::HoundeyeGibs( pev, 1, 0 ); 
		ALERT ( at_console, ">Cortando middle_LEFT \n" );
	}
	else if( m_LastHitGroup == HOUNDEYE_RIGHTLEG)
	{
...
void CHoundeye :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	...
	if ( (flDamage >= 40 && pev->health <= 25) || flDamage >= 70 )
	{	
		if (ptr->iHitgroup == HITGROUP_HEAD) // si el daño es en la cabeza
		{
			UTIL_BloodStream( ptr->vecEndPos, gpGlobals->v_forward * -5 + gpGlobals->v_up * 2, (unsigned short)58, 80 ); //color * cantidad
		}
	
		if (ptr->iHitgroup == HOUNDEYE_LEFTLEG || HOUNDEYE_RIGHTLEG || HOUNDEYE_MIDDLELEG)
		{	
			UTIL_BloodStream( ptr->vecEndPos, gpGlobals->v_forward * -5 + gpGlobals->v_up * 2, (unsigned short)58, 50 ); //color * cantidad
		}
	}
