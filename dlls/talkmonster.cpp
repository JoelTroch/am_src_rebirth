...
char *CTalkMonster::m_szFriends[TLK_CFRIENDS] = 
{
	...
	"monster_hgrunt_ally",
	"monster_otis",
	"monster_scientist_barra",
	"monster_agency_member",
};
...
Task_t	tlIdleResponse[] =
{
	...
	{ TASK_SET_ACTIVITY,	(float)ACT_IDLE /*SYS*/	},
	{ TASK_TLK_EYECONTACT,	(float)0		},// Wait until speaker is done
};
...
{
	{ TASK_SET_ACTIVITY,	(float)ACT_IDLE /*SYS*/	},// Stop and talk
	{ TASK_TLK_SPEAK,		(float)0		},// question or remark
	{ TASK_TLK_EYECONTACT,	(float)0		},// 
...
void CTalkMonster :: SetActivity ( Activity newActivity )
{
	if (newActivity == ACT_IDLE && IsTalking() )
		newActivity = ACT_IDLE /*SYS*/;
	
	if ( newActivity == ACT_IDLE /*SYS*/ && (LookupActivity ( ACT_IDLE /*SYS*/ ) == ACTIVITY_NOT_AVAILABLE))
	...
	for ( i = 0; i < TLK_CFRIENDS; i++ )
	{
		while (pFriend = EnumFriends( pFriend, i, FALSE ))
		{
			...
			if ( pMonster )
			{
				...
				{
					count++;
//					if ( count > maxFollowers )//sys fix: max followers becoming 1 when the player use a ent, change this
...
void CTalkMonster :: TalkInit( void )
{
	...
	if (m_iszSpeakAs) //LRC: changing voice groups for monsters
	{
		...
		strcpy(szAssign,"PAIN");
		m_szGrp[TLK_PAIN]  = STRING(ALLOC_STRING(szBuf)); //SP
	}
...
/*
	...
	if (FClassnameIs(pev, "monster_agency_member"))
	{
		while ((pEntity = UTIL_FindEntityInSphere( pEntity, VecSrc, 512 )) != NULL)//512
		{
			...
			{
				EMIT_SOUND_DYN( ENT(pev), CHAN_WEAPON, "npc_voices/friend/radio/SuspectCooperate.wav", 1, ATTN_NORM, 0, 100 );
			}
		}
	}
	else
...
void CTalkMonster::PlaySentence( const char *pszSentence, float duration, float volume, float attenuation )
{
	...
	{
		if ( pszSentence[0] == '!' )
			EMIT_SOUND_DYN( edict(), CHAN_VOICE, pszSentence, volume, attenuation, 0, 50);
		else
			SENTENCEG_PlayRndSz( edict(), pszSentence, volume, attenuation, 0, 50 );
	}
	else
	{
		if ( pszSentence[0] == '!' )
...
//		ALERT ( at_aiconsole, "Clear smell bits\n" );
	...
	else if ( FClassnameIs( pev, "monster_scientist_barra" ) ) {}
	else if ( FClassnameIs( pev, "monster_barney" ) ) {}
	else if ( FClassnameIs( pev, "monster_otis" ) ) {}
	else if ( FClassnameIs( pev, "monster_friend" ) ) {}
	else if ( FClassnameIs( pev, "monster_scientist_barra" ) ) {}
	else
	{
		ClientPrint(pPlayer->pev, HUD_PRINTTALK, "Equipo aliado (RADIO): quien manda aca" );
	}
*/
	}
}
...
void CTalkMonster::StopFollowing( BOOL clearSchedule )
{
	...
	{
		if (RANDOM_LONG(0,1))
		ClientPrint(pPlayer->pev, HUD_PRINTTALK, "Equipo aliado (RADIO): me quedo aca" );
		else
		ClientPrint(pPlayer->pev, HUD_PRINTTALK, "Equipo aliado (RADIO): e quedo aca2" );
	}
*/
	}
}
...
void CTalkMonster::StartFollowing( CBaseEntity *pLeader )
{
	...
	ClearSchedule();
/*
//Skip all another folower entities
		CBaseEntity *pPlayer = UTIL_PlayerByIndex( 1 );	
	...
	{
		if (RANDOM_LONG(0,1))
		ClientPrint(pPlayer->pev, HUD_PRINTTALK, "Equipo aliado (RADIO): A sus ordenes" );
		else
		ClientPrint(pPlayer->pev, HUD_PRINTTALK, "Equipo aliado (RADIO): Lo que diga" );
	}
*/
}
