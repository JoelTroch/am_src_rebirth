...
*small modification to suppor hl2 model combine turret
...
void CBaseTurret::Ping( void )
{
	...
	else if (m_flPingTime <= gpGlobals->time)
	{
		...
		EMIT_SOUND(ENT(pev), CHAN_ITEM, "turret_combine/ping.wav", 1, ATTN_NORM);
		else
...
void CBaseTurret::Deploy(void)
{
	...
	if (pev->sequence != TURRET_ANIM_DEPLOY)
	{
		...
		if (pev->frags)//HL2 model compatibility
		EMIT_SOUND(ENT(pev), CHAN_ITEM, "turret_combine/deploy.wav", 1, ATTN_NORM);
		else
...
void CTurret::SpinUpCall(void)
{
	...
	if (!m_iSpin)
	{
		...
		if (!m_iStartSpin)
		{
			...
			if (pev->frags)//HL2 model compatibility
			EMIT_SOUND(ENT(pev), CHAN_ITEM, "turret_combine/active.wav", 1, ATTN_NORM);
			else
...
void CBaseTurret::AutoSearchThink(void)
{
	...
	if (m_hEnemy != NULL)
	{
		...
		if (pev->frags)//HL2 model compatibility
		EMIT_SOUND(ENT(pev), CHAN_ITEM, "turret_combine/alarm.wav", 1, ATTN_NORM);
		else
...
{ 
	...
	if (pev->frags)//HL2 model compatibility
	{
		...
		SET_MODEL(ENT(pev), "models/hl2/Combine_Turret.mdl");
	}
	else
	{
		if (pev->model)
...
class CCombineSentry : public CSentry
{
public:
	void Spawn( );
	void Precache(void);
};
LINK_ENTITY_TO_CLASS( npc_combine_turret, CCombineSentry );

void CCombineSentry::Precache()
{
	CBaseTurret::Precache( );

	PRECACHE_MODEL("models/hl2/Combine_Turret.mdl");

	PRECACHE_SOUND ("turret_combine/shoot1.wav");
	PRECACHE_SOUND ("turret_combine/shoot2.wav");
	PRECACHE_SOUND ("turret_combine/shoot3.wav");

	PRECACHE_SOUND ("turret_combine/ping.wav");
	PRECACHE_SOUND ("turret_combine/deploy.wav");
	PRECACHE_SOUND ("turret_combine/alert.wav");

	PRECACHE_SOUND ("turret_combine/die.wav");
}

void CCombineSentry::Spawn()
{ 
	Precache( );

	pev->frags = 1;//HL2 model
...
void CSentry::Shoot(Vector &vecSrc, Vector &vecDirToEnemy)
{
	...
	if (pev->frags)//HL2 model compatibility
	{
		switch(RANDOM_LONG(0,2))
		{
		case 0: EMIT_SOUND(ENT(pev), CHAN_WEAPON, "turret_combine/shoot1.wav", 1, ATTN_NORM); break;
		case 1: EMIT_SOUND(ENT(pev), CHAN_WEAPON, "turret_combine/shoot2.wav", 1, ATTN_NORM); break;
		case 2: EMIT_SOUND(ENT(pev), CHAN_WEAPON, "turret_combine/shoot3.wav", 1, ATTN_NORM); break;
		}
	}
	else
	{
...
void CSentry ::	SentryDeath( void )
{
	...
	if (pev->deadflag != DEAD_DEAD)
	{
		...
		if (pev->frags)//HL2 model compatibility
		{
			EMIT_SOUND(ENT(pev), CHAN_BODY, "turret_combine/die.wav", 1.0, ATTN_NORM);
		}
		else
		{
			if ( flRndSound <= 0.33 )
