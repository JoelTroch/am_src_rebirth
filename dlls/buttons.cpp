...
+func_tbutton -CODE BY Overground Team- (go ahead and translate the following :P-
 ...
 Bouton spécial qui peut mettre un certain temps à s'activer ou se désactiver
 Contrairement à ceux d'origines dans HL, ils ne peuvent pas bouger, ni se faire
...
void CFuncTButton::Spawn( void )
{
	/*
	 ...
	 On est obligé de mettre un solid_bbox au lieu d'un solid_bsp
	 tout simplement parceque solid_bsp implique movetype_push
	 et que la movetype_push implique lui même que le Think ne soit
	 lancé que quand l'objet bouge, alors on fait un vieux HACK
	 pour permettre au bouton d'avoir des decals quand même !
...
void CFuncTButton::KeyValue( KeyValueData *pkvd )
{
	...
	// Cible lors de la désactivation
...
void CFuncTButton::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	// Si c'est la première fois qu'on cherche à l'utiliser, alors
	...
	if ( !b_in_use )
	{
		...
		// Et on sauvegarde le temps à ce moment
		f_starttime = gpGlobals->time;

		// Et le joueur qui cherche à activer l'entité
	...
	// On cherche à activer l'animal
	if ( !m_bState )
	{
		// On verifie si le bouton doit être activé
		if ( ( f_starttime + f_on_time ) <= gpGlobals->time )
		{
			// On change l'état
			m_bState = true;

			// On revoie un message au cas où ça est merdé
			if ( f_on_time > 0 )
			{
			...
			//SysOp: Active le targetè au sendeerè má'ssegé (jejeje... I'm a French boy)
			UTIL_ShowMessage( STRING(pev->message), pActivator );

			f_lastacttime	= gpGlobals->time;
			last_owner		= pev->owner;

			// Le bouton n'est plus utilisé
			b_in_use		= false;
			f_starttime		= 0.0;
			pev->owner		= NULL;
	...
	// On veut le désactiver
	else
	{
		// On verifie si le bouton doit être désactivé
...
void CFuncTButton::BThink( void )
{
	...
	// On vérifie ici si le bonhomme à pas laché le bouton
	if ( pev->owner && CBaseEntity::Instance( pev->owner ) )
	{
		// On fait bien attention maintenant à ce que le joueur voit le bouton
