...
// overrideable Monster member functions
	...
	void EXPORT BounceCorpse( CBaseEntity *pOther );
	void NormalizeAngles( float *angles );
	void LimitVelocity( void );

	BOOL f_bNearSmoke;
	float f_NextBloodTime;
		
	float f_NextBurnTime;
	float f_NextSoundBurnTime;

	BOOL VisionCheck( void );
	BOOL CanSee(CBaseEntity *pLooker, CBaseEntity *pSeen);
/*
	float m_flPlayerSmokeTime; //the variable which holds the smoke time
	void SmokeTrail( void ); //we need to define this function before we can call/write it
*/
	int					m_iClass;
	int					m_iPlayerReact;
...
//		BOOL FCheckMonsterMakerAITrigger( void );// checks and, if necessary, fires the monster's trigger target. 
...
//		void MakeImpactEffect ( bool IsPlayer );
	...
	void StartPatrol( CBaseEntity *path );//LRC add on
