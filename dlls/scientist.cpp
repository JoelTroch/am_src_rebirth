...
#define HEAD_GLASSES_DEAD			5
#define HEAD_EINSTEIN_DEAD			6
#define HEAD_LUTHER_DEAD			7
#define HEAD_SLICK_DEAD				8
*/
...
void CScientist :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	//	pev->body = (oldBody % NUM_SCIENTIST_HEADS) + NUM_SCIENTIST_HEADS * 1;
			SetBodygroup( NEDLE_GROUP, NEDLE_ON );
		}
		break;
	...
	//	pev->body = (oldBody % NUM_SCIENTIST_HEADS) + NUM_SCIENTIST_HEADS * 0;
			SetBodygroup( NEDLE_GROUP, NEDLE_OFF );
		}
		break;
...
//	PRECACHE_MODEL ( "models/w_needle.mdl" ); // para la jeringa
...
void CScientist :: TalkInit()
{
	...
	m_szFriends[2] = "monster_ct_m4a1_ally";
	...
	if (!m_iszSpeakAs)
	{
		...
		//Teniendo en cuenta que "Change the prefix for all this monster's speeches"
		//ok, hagamos nuevo grupo para Customs Pain Sound y vemos q pasa.
		m_szGrp[TLK_PAIN] =		"SC_PAIN";//SP - untested
	}
...
//	PlaySentence( "SC_PAIN", RANDOM_FLOAT(3, 6), VOL_NORM, ATTN_NORM );
...
/*
	...
	{
		if (GetBodygroup( 1 ) == HEAD_GLASSES)
		SetBodygroup( HEAD_GROUP, HEAD_GLASSES_DEAD );

		if (GetBodygroup( 1 ) == HEAD_EINSTEIN)
		SetBodygroup( HEAD_GROUP, HEAD_EINSTEIN_DEAD );

		if (GetBodygroup( 1 ) == HEAD_LUTHER)
		SetBodygroup( HEAD_GROUP, HEAD_LUTHER_DEAD );

		if (GetBodygroup( 1 ) == HEAD_SLICK)
		SetBodygroup( HEAD_GROUP, HEAD_SLICK_DEAD );

		ALERT( at_console, "> Setting Head Model... Was Headshot! \n" );
	}
	*/
}
...
//	pev->movetype		= MOVETYPE_STEP;
	pev->movetype		= MOVETYPE_NONE;//Sys- why step? the scientist doens't move! This can preven the monster stuck at floor
...
//	DROP_TO_FLOOR ( ENT(pev) );//dont drop
}
...
//=========================================================
// TraceAttack - checks to see if was shot in the head
//=========================================================
void CScientist :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	...
	{
		ALERT( at_console, "Scientist Headshot!\n" );
