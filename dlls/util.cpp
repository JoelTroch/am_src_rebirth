...
+cl_autohelp on client print (if 0 the user can't recieve messages)
+UTIL_WhiteSparks
...
// On ne l'applique qu'a un seul joueur, utilser pour les armes entre autre
void UTIL_ScreenShakeOne( CBasePlayer *pPlayer, float amplitude, float frequency, float duration )
{
//	if ( pPlayer->PlayerShake )
//	{
		float		localAmplitude;
		ScreenShake	shake;
		...
		// Si y'a pas de joueur on fait rien, logique
		if ( !pPlayer )
			return;
...
{
	if ( CVAR_GET_FLOAT( "cl_autohelp" ) >= 1 )
	{
...
/*
		MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, origin );
			...
			WRITE_COORD( origin.y);
			WRITE_COORD( origin.z);
			WRITE_SHORT( g_sModelIndexNull );				// just we will use drops!
...
void CSave :: WriteFunction( const char *pname, const int *data, int count )
{
	...
	if ( functionName )
		...
		ALERT( at_error, "Invalid function pointer in entity! %i",functionName );
}
...
void UTIL_SetModel( edict_t *e, const char *model )
{
	...
	{
		//this is sprite
		g_engfuncs.pfnSetModel(e, "models/common/error.spr");
	}
	else
	{
		//set null model
...
{ return UTIL_PrecacheModel( (char*)STRING(s)); }
