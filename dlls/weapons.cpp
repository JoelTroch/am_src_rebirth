...
+S2P Physics
+save/restore values
+functions to strip weapons and ammo -by LRC-
+modeles, sounds, sprites to be precached
...
DLL_GLOBAL	short	g_sModelIndexFireball2;// holds the index for the fireball//sys
DLL_GLOBAL	short	g_sModelIndexFireball3;// holds the index for the fireball
DLL_GLOBAL	short	g_sModelIndexFireballEnd;// holds the index for the fireball//sys

DLL_GLOBAL	short	g_sGrenadeGib;// holds the index for the fireball
...
//	ALERT (at_console, "Some monster is trying to decal a wall!\n");
...
void W_Precache(void)
{
	...
	UTIL_PrecacheOther( "xp_point_brown" );
	UTIL_PrecacheOther( "xp_point_gold" );
	UTIL_PrecacheOther( "xp_point_plate" );
	...
	//this should be in each weapon... well i'm so tired to do it at the moment...
	//there is no real need to precache them, but some sounds are used by monsters
	
	PRECACHE_SOUND( "weapons/deagle/deagle_fire-1.wav" );
	PRECACHE_SOUND( "weapons/sniper/sniper_fire-1.wav" );
	PRECACHE_SOUND( "weapons/sniper/sniper_fire-2.wav" );
	PRECACHE_SOUND( "weapons/m249/M249_fire-1.wav" );
	PRECACHE_SOUND( "weapons/m249/M249_fire-2.wav" );
	PRECACHE_SOUND( "weapons/mp5/mp5_fire-1.wav" );
	PRECACHE_SOUND( "weapons/mp5/mp5_fire-2.wav" );
	PRECACHE_SOUND( "weapons/glock/glock_fire-1.wav" );
	PRECACHE_SOUND( "weapons/glock/glock_fire-2.wav" );
	PRECACHE_SOUND( "weapons/m16/m16_fire-1.wav" );
	PRECACHE_SOUND( "weapons/m16/m16_fire-2.wav" );
	PRECACHE_SOUND( "weapons/shotgun/sbarrel1.wav" );
	PRECACHE_SOUND( "weapons/shotgun/dbarrel1.wav" );
	...
	g_sModelIndexNull = PRECACHE_MODEL ("sprites/null.spr"); // splattered blood 

	g_sModelIndexLaser = PRECACHE_MODEL( (char *)g_pModelNameLaser );

	g_sModelIndexLaserDot = PRECACHE_MODEL("sprites/laserdot.spr");

//	PRECACHE_MODEL("sprites/laser_small.spr");//SP: Used by weapons
//	PRECACHE_MODEL("sprites/flash_beam.spr");//SP: Used by flash light

	PRECACHE_SOUND ("weapons/bulletimpact/concrete1.wav");
	PRECACHE_SOUND ("weapons/bulletimpact/concrete2.wav");
	PRECACHE_SOUND ("weapons/bulletimpact/concrete3.wav");

	PRECACHE_SOUND ("weapons/bulletimpact/glass1.wav");
	PRECACHE_SOUND ("weapons/bulletimpact/glass2.wav");
	PRECACHE_SOUND ("weapons/bulletimpact/glass3.wav");

	PRECACHE_SOUND ("weapons/bulletimpact/wood1.wav");
	PRECACHE_SOUND ("weapons/bulletimpact/wood2.wav");
	PRECACHE_SOUND ("weapons/bulletimpact/wood3.wav");

	PRECACHE_SOUND ("weapons/bulletimpact/tile1.wav");
	PRECACHE_SOUND ("weapons/bulletimpact/tile2.wav");
	PRECACHE_SOUND ("weapons/bulletimpact/tile3.wav");

	PRECACHE_SOUND ("weapons/bulletimpact/metal1.wav");
	PRECACHE_SOUND ("weapons/bulletimpact/metal2.wav");
	PRECACHE_SOUND ("weapons/bulletimpact/metal3.wav");
	...
	PRECACHE_SOUND ("weapons/whizz5.wav");
	...
	g_sGrenadeGib = PRECACHE_MODEL("models/weapons/grenade_gibs.mdl");

	// used by explosions
	PRECACHE_MODEL ("models/grenade.mdl");

	// del flash (el jugador y los monsters lo usan)
	PRECACHE_MODEL ("models/weapons/w_flashbang.mdl");

	PRECACHE_SOUND ("weapons/bang.wav");// explosion aftermaths
	...
	PRECACHE_SOUND ("items/weapondrop2.wav");// weapon falls to the ground
}
...
//	pev->movetype = MOVETYPE_TOSS;
	...
	{
		if ( phys_objects.value != 0  ) //no es cero esta _Activado_
		pev->movetype = MOVETYPE_BOUNCE;
		else
		pev->movetype = MOVETYPE_TOSS;
	}

	pev->solid = SOLID_TRIGGER;//SOLID_BBOX //sys test
	...
	//test
//	UTIL_SetSize(pev, Vector( 50, 50, 50), Vector(50, 50, 50) );//pointsize until it lands on the ground.
...
void CBasePlayerItem::FallThink ( void )
{
	...
	if ( pev->flags & FL_ONGROUND )
	{
		...
		if ( !FNullEnt( pev->owner ) )
		{
			...
			if (b_iNextWeaponDropSound < gpGlobals->time)
			{
	...
	/*
				...
				b_iNextWeaponDropSound = gpGlobals->time + 1;
			}
		}

		// lie flat
		//cambiar esto para hacer al azar???
		pev->angles.x = 0;
		pev->angles.z = 0;
	...
	//sys test
//	pev->velocity = pev->velocity * 0.8;// weaponbox has player's velocity, then some.
}
...
void CBasePlayerItem :: NormalizeAngles( float *angles )
{
	int i;
	// Normalize angles
	for ( i = 0; i < 3; i++ )
...
void CBasePlayerItem::BounceCode()//FIX
{
/*
	TraceResult tr;
	Vector		vecSrc;
...
//	TraceResult tr_reverse;

//	UTIL_TraceHull( pev->origin, pev->origin + Vector( 32, 32, 32 ), ignore_monsters, head_hull, edict(), &tr );
		
	// make sure there's room to fall forward
	UTIL_TraceHull ( vecSrc, vecSrc + gpGlobals->v_forward * 16 - gpGlobals->v_right * 16 + gpGlobals->v_up * 16, ignore_monsters, head_hull, edict(), &tr );
...
//	ALERT( at_console, "flFraction is %f\n", tr.flFraction );
		...
		if ( phys_enable.value != 0  ) //no es cero esta _Activado_
		{
			...
			//-?
					...
					pev->angles.z = UTIL_VecToAngles( angdiry - DotProduct(angdiry, tr.vecPlaneNormal) * tr.vecPlaneNormal).z;//cuidado
				}
			}
		}
...
//			int iXPPoints = CVAR_GET_FLOAT( "xp_points" );
			...
			ClientPrint(pPlayer->pev, HUD_PRINTCENTER, "#PressUseForPickUp"); //digamos al cliente
			SERVER_COMMAND("nearweapon 1\n");

//			if (FClassnameIs( pev, STRING( this->pev->classname) ))
//			{
//				if (!b_WpDeagle_Buyed)
//				{
//					ClientPrint(pPlayer->pev, HUD_PRINTCENTER, "Presiona usar para comprar\n1240 XP necesarios"); //digamos al cliente
//			
//					//si esta presionado, seguir funcion
...
//					if (iXPPoints < 1240)
//					{
//						ClientPrint(pPlayer->pev, HUD_PRINTCENTER, "No tienes XP suficiente!"); //digamos al cliente
//						return;
//					}
//					else
...
//				}
//				//else
//				//el arma esta comprada, pasemos por alto esto
//			}
...
void CBasePlayerWeapon::UpdateItemInfo( void )
{
    ...
    if (GetItemInfo(&iInfo))
    {
        if (iInfo.weaponName)
        {
          ...
          //sprintf(szText, "%s seleccionada...", iInfo.weaponName);  
...
void CWeaponBox::Spawn( void )
{
	...
	pev->solid = SOLID_TRIGGER;//bbox later?

	pev->takedamage = DAMAGE_YES;//sys test
	pev->health = 99999;//sys test

	UTIL_SetSize( pev, g_vecZero, g_vecZero );

	SET_MODEL( ENT(pev), "models/w_weaponbox.mdl");

//	SetThink(&CWeaponBox :: BounceThink );
	pev->nextthink = gpGlobals->time + 0.1;
}

void CWeaponBox::BounceCode()
{
//	ALERT( at_console, "BounceCode\n" );
...
void CWeaponBox :: NormalizeAngles( float *angles )
{
	int i;
	// Normalize angles
	for ( i = 0; i < 3; i++ )
...
void CWeaponBox :: WeaponBoxThink( void )
{	//FIX
//	bool TraceLineFront, TraceLineBack;
/*
	TraceResult tr;
	Vector		vecSrc;
...
//	if (tr_reverse.fStartSolid || tr_reverse.flFraction < 1.0)
//	TraceLineBack = TRUE;
...
//	ALERT ( at_console, "WeaponBoxThink...\n" );
//	StudioFrameAdvance( );
	pev->nextthink = gpGlobals->time + 0.1;

	//this dont work (think)
	if (!IsInWorld())
	{
		UTIL_Remove( this );
		return;
...
{
//	ALERT ( at_console, "CWeaponBox :: TakeDamage\n" );
	pev->dmg += flDamage;

	ALERT(at_console,"Your weapon is being attacked!!\n");

	pev->flags &= ~FL_ONGROUND;
	pev->origin.z += 2;
	pev->velocity = g_vecAttackDir * -1;
	pev->velocity = pev->velocity * RANDOM_FLOAT( 500, 600 );
				
	pev->avelocity.x = RANDOM_FLOAT( -50, 50 );
	pev->avelocity.y = RANDOM_FLOAT( -50, 50 );
	pev->avelocity.z = RANDOM_FLOAT( -50, 50 );
...
/*
			...
			vecSrc = pev->origin + vecForward * RANDOM_FLOAT( -8, 8 ) + vecRight * RANDOM_FLOAT( -8, 8 ) + vecUp * RANDOM_FLOAT( 2, 8 );//!!-32??

			CSprite *pSprite = CSprite::SpriteCreate( "sprites/exp_end.spr", vecSrc, TRUE );
					
			pSprite->SetTransparency( kRenderTransAlpha, 222, 222, 222, 88, kRenderFxNone );
			pSprite->SetScale( 5.0 );
			pSprite->Expand( 5 , RANDOM_FLOAT( 140.0, 150.0 )  );
			pSprite->pev->frame = 0;
	*/	
			CSoundEnt::InsertSound ( bits_SOUND_COMBAT, pev->origin, 1000, 0.3 );
			...
			{
				if (RANDOM_LONG(0,1))
					EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "weapons/cbar_hit1.wav", 0.5, ATTN_NORM, 0, pitch);
				else
					EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "weapons/cbar_hit2.wav", 0.5, ATTN_NORM, 0, pitch);
			}
			else if(bDoHESound)
			{
				switch (RANDOM_LONG(0,2)) //ejecuta los siguientes sonidos al azar
				{
					case 0: EMIT_SOUND (ENT(pev), CHAN_ITEM, "weapons/grenade_hit1.wav", .5, ATTN_NORM ); break;
					case 1: EMIT_SOUND (ENT(pev), CHAN_ITEM, "weapons/grenade_hit2.wav", .5, ATTN_NORM ); break;
					case 2: EMIT_SOUND (ENT(pev), CHAN_ITEM, "weapons/grenade_hit3.wav", .5, ATTN_NORM ); break;
				}
			}
			else
			{
...
BOOL CWeaponBox::PackWeapon( CBasePlayerItem *pWeapon )
{
	...
	pWeapon->pev->solid = SOLID_NOT;//solid_not?
...
//////////////////////
// the "real" thing"
	if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_knife")))
//	{
...
//	}
	...
	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_tripmine")))
	{
		...
		SET_MODEL( ENT(pev), "models/weapons/tripmine/v_tripmine.mdl");
	}
	...
	SET_MODEL( ENT(pev), "models/weapons/satchel/w_satchel.mdl"); 

	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_m40a1")))
	SET_MODEL( ENT(pev), "models/weapons/sniper/w_sniper.mdl");

	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_eagle")))
	SET_MODEL( ENT(pev), "models/weapons/deagle/w_deagle.mdl");

	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_saw")))
	{
	...
	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_m16")))
	SET_MODEL( ENT(pev), "models/weapons/m16/w_m16.mdl");

	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_m16")))
	SET_MODEL( ENT(pev), "models/weapons/m16/w_m16.mdl");

	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_crowbar")))
	{
		SET_MODEL( ENT(pev), "models/weapons/crowbar/w_crowbar.mdl");
		bDoCrowbarSound = TRUE;
	}

	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_medkit")))
...
//--------------
//MONSTER HAND GRENADE HACK
//--------------
#define SF_GRENADE_ON 1

class CHandGrenadeMonster : public CBaseMonster
{
public:
	void Spawn( void );
	void Precache( void );
...
void CHandGrenadeMonster :: Spawn()
{
	Precache( );
	...
	if (pev->spawnflags & SF_GRENADE_ON)
	CGrenade::ShootTimed( pev, pev->origin, pev->angles, 3.5 );

	pev->nextthink = gpGlobals->time + 0.1;
}
LINK_ENTITY_TO_CLASS( monster_he_grenade, CHandGrenadeMonster );
LINK_ENTITY_TO_CLASS( monster_handgrenade, CHandGrenadeMonster );

void CHandGrenadeMonster :: Precache()
{	
//No precaches needed because these models are used by weapons
}

void CHandGrenadeMonster::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{	
	CGrenade::ShootTimed( pev, pev->origin, pev->angles, 3.5 );
...
//--------------
class CSmokeGrenadeMonster : public CBaseMonster
{
public:
	void Spawn( void );
	void Precache( void );
...
void CSmokeGrenadeMonster :: Spawn()
{
	Precache( );
	...
	if (pev->spawnflags & SF_GRENADE_ON)
	CGrenade::ShootTimedSmoke( pev, pev->origin, pev->angles, 25 );

	pev->nextthink = gpGlobals->time + 0.1;
}
LINK_ENTITY_TO_CLASS( monster_smoke_grenade, CSmokeGrenadeMonster );

void CSmokeGrenadeMonster :: Precache()
{	
//No precaches needed because these models are used by weapons
}

void CSmokeGrenadeMonster::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{	
	CGrenade::ShootTimedSmoke( pev, pev->origin, pev->angles, 25 );
...
TYPEDESCRIPTION CFlash::m_SaveData[] =
{
DEFINE_FIELD( CFlash, m_fSpotActive, FIELD_INTEGER ),
DEFINE_FIELD( CFlash, m_pBeam, FIELD_CLASSPTR ),
//new values
//	DEFINE_FIELD( CFlash, m_FlashGlow, FIELD_CLASSPTR ),
//	DEFINE_FIELD( CFlash, m_iFlashBrightness, FIELD_INTEGER ),
};
IMPLEMENT_SAVERESTORE( CFlash, CBasePlayerWeapon );

TYPEDESCRIPTION	CHL2Shotgun::m_SaveData[] = 
{
	DEFINE_FIELD( CHL2Shotgun, m_flNextReload, FIELD_TIME ),
	DEFINE_FIELD( CHL2Shotgun, m_fInSpecialReload, FIELD_INTEGER ),
	DEFINE_FIELD( CHL2Shotgun, m_flNextReload, FIELD_TIME ),
	DEFINE_FIELD( CHL2Shotgun, m_flPumpTime, FIELD_TIME ),
};
IMPLEMENT_SAVERESTORE( CHL2Shotgun, CBasePlayerWeapon );

TYPEDESCRIPTION CDeagle::m_SaveData[] =
{
DEFINE_FIELD( CDeagle, m_fSpotActive, FIELD_INTEGER ),
DEFINE_FIELD( CDeagle, m_pBeam, FIELD_CLASSPTR ),
};
