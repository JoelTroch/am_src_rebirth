...
//=========================================================
//=========================================================
class CBirdFlyerFlock : public CBaseMonster
{
public:
	void Spawn( void );
	void Precache( void );
...
TYPEDESCRIPTION	CBirdFlyerFlock::m_SaveData[] = 
{
	DEFINE_FIELD( CBirdFlyerFlock, m_cFlockSize, FIELD_INTEGER ),
	DEFINE_FIELD( CBirdFlyerFlock, m_flFlockRadius, FIELD_FLOAT ),
};

IMPLEMENT_SAVERESTORE( CBirdFlyerFlock, CBaseMonster );

//=========================================================
//=========================================================
class CBirdFlyer : public CBaseMonster
{
public:
	void Spawn( void );
	void Precache( void );
	...
	int	SquadCount( void );
	void SquadRemove( CBirdFlyer *pRemove );
	void SquadUnlink( void );
	void SquadAdd( CBirdFlyer *pAdd );
	void SquadDisband( void );

	CBirdFlyer *m_pSquadLeader;
	CBirdFlyer *m_pSquadNext;
...
LINK_ENTITY_TO_CLASS( monster_bird, CBirdFlyer );
LINK_ENTITY_TO_CLASS( monster_bird_flock, CBirdFlyerFlock );

TYPEDESCRIPTION	CBirdFlyer::m_SaveData[] = 
{
	DEFINE_FIELD( CBirdFlyer, m_pSquadLeader, FIELD_CLASSPTR ),
	DEFINE_FIELD( CBirdFlyer, m_pSquadNext, FIELD_CLASSPTR ),
	DEFINE_FIELD( CBirdFlyer, m_fTurning, FIELD_BOOLEAN ),
	DEFINE_FIELD( CBirdFlyer, m_fCourseAdjust, FIELD_BOOLEAN ),
	DEFINE_FIELD( CBirdFlyer, m_fPathBlocked, FIELD_BOOLEAN ),
	DEFINE_FIELD( CBirdFlyer, m_vecReferencePoint, FIELD_POSITION_VECTOR ),
	DEFINE_FIELD( CBirdFlyer, m_vecAdjustedVelocity, FIELD_VECTOR ),
	DEFINE_FIELD( CBirdFlyer, m_flGoalSpeed, FIELD_FLOAT ),
	DEFINE_FIELD( CBirdFlyer, m_flLastBlockedTime, FIELD_TIME ),
	DEFINE_FIELD( CBirdFlyer, m_flFakeBlockedTime, FIELD_TIME ),
	DEFINE_FIELD( CBirdFlyer, m_flAlertTime, FIELD_TIME ),
//	DEFINE_FIELD( CBirdFlyer, m_flFlockNextSoundTime, FIELD_TIME ),	// don't need to save
};

IMPLEMENT_SAVERESTORE( CBirdFlyer, CBaseMonster );

//=========================================================
//=========================================================
void CBirdFlyerFlock :: KeyValue( KeyValueData *pkvd )
{
...
void CBirdFlyerFlock :: PrecacheFlockSounds( void )
{
	PRECACHE_SOUND("bird/bird_alert1.wav" );
	PRECACHE_SOUND("bird/bird_alert2.wav" );

	PRECACHE_SOUND("bird/bird_idle1.wav" );
	PRECACHE_SOUND("bird/bird_idle2.wav" );
}

//=========================================================
//=========================================================
void CBirdFlyerFlock :: SpawnFlock( void )
{
	float R = m_flFlockRadius;
	int iCount;
	Vector vecSpot;
	CBirdFlyer *pBoid, *pLeader;

	pLeader = pBoid = NULL;

	for ( iCount = 0 ; iCount < m_cFlockSize ; iCount++ )
	{
		pBoid = GetClassPtr( (CBirdFlyer *)NULL );
		...
		pBoid->SetThink( CBirdFlyer :: IdleThink );
...
void CBirdFlyer :: Spawn( )
{
	...
	SetThink( IdleThink );
//	if (pev->frags != 1) //si no esta seteado el NO SOUND, entonces habilitarlo
//		pev->frags	= 0;
...
void CBirdFlyer :: Precache( )
{
	...
	CBirdFlyerFlock::PrecacheFlockSounds();
}

//=========================================================
//=========================================================
void CBirdFlyer :: MakeSound( void )
{
	if ( pev->spawnflags & SF_MONSTER_GAG )
	{
		ALERT (at_console, "#Birds Can't Make Sounds! Entity Was Set! \n");
		return;
	}
	else
	{
		if ( m_flAlertTime > gpGlobals->time )
		{
			...
			{
				case 0:	EMIT_SOUND( ENT(pev), CHAN_WEAPON, "bird/bird_alert1.wav", 1, ATTN_NORM );	break;
				case 1:	EMIT_SOUND( ENT(pev), CHAN_WEAPON, "bird/bird_alert2.wav", 1, ATTN_NORM );	break;
			}
		...
		switch ( RANDOM_LONG( 0, 1 ) )
		{
			case 0:	EMIT_SOUND( ENT(pev), CHAN_WEAPON, "bird/bird_idle1.wav", 1, ATTN_NORM );	break;
			case 1:	EMIT_SOUND( ENT(pev), CHAN_WEAPON, "bird/bird_idle2.wav", 1, ATTN_NORM );	break;
		}
	}
}
...
//=========================================================
//=========================================================
void CBirdFlyer :: Killed( entvars_t *pevAttacker, int iGib )
{
	CBirdFlyer *pSquad;
	
	pSquad = (CBirdFlyer *)m_pSquadLeader;

	while ( pSquad )
	{
		pSquad->m_flAlertTime = gpGlobals->time + 15;
		pSquad = (CBirdFlyer *)pSquad->m_pSquadNext;
	}
...
//=========================================================
//=========================================================
void CBirdFlyer :: SpawnCommonCode( )
{
	pev->deadflag	= DEAD_NO;
	pev->classname	= MAKE_STRING("monster_bird");
...
//=========================================================
//=========================================================
void CBirdFlyer :: BoidAdvanceFrame ( )
{
...
//=========================================================
//=========================================================
void CBirdFlyer :: IdleThink( void )
{
...
//=========================================================
void CBirdFlyer :: FormFlock( void )
{
	if ( !InSquad() )
	{
		// I am my own leader
		...
		while ((pEntity = UTIL_FindEntityInSphere( pEntity, pev->origin, AFLOCK_MAX_RECRUIT_RADIUS )) != NULL)
		{
			...
			if ( pRecruit && pRecruit != this && pRecruit->IsAlive() && !pRecruit->m_pCine )
			{
				...
				if ( FClassnameIs ( pRecruit->pev, "monster_bird" ) )
				{
					squadCount++;
					SquadAdd( (CBirdFlyer *)pRecruit );
				}
			}
		}
	}
...
//=========================================================
void CBirdFlyer :: FlockLeaderThink( void )
{
	TraceResult		tr;
	Vector			vecDist;// used for general measurements
	Vector			vecDir;// used for general measurements
...
//=========================================================
void CBirdFlyer :: FlockFollowerThink( void )	
{
	TraceResult		tr;
	Vector			vecDist;
	Vector			vecDir;
...
//
//=========================================================
void CBirdFlyer :: SquadUnlink( void )
{
	m_pSquadLeader = NULL;
	m_pSquadNext	= NULL;
}
...
//
//=========================================================
void CBirdFlyer :: SquadAdd( CBirdFlyer *pAdd )
{
	ASSERT( pAdd!=NULL );
	ASSERT( !pAdd->InSquad() );
...
//
//=========================================================
void CBirdFlyer :: SquadRemove( CBirdFlyer *pRemove )
{
	ASSERT( pRemove!=NULL );
	ASSERT( this->IsLeader() );
	...
	if ( SquadCount() > 2 )
	{
		...
		if ( pRemove == this )
		{
			CBirdFlyer *pLeader = m_pSquadNext;
...
//
//=========================================================
int CBirdFlyer :: SquadCount( void )
{
	CBirdFlyer *pList = m_pSquadLeader;
	int squadCount = 0;
	while ( pList )
...
//
//=========================================================
void CBirdFlyer :: SquadDisband( void )
{
	CBirdFlyer *pList = m_pSquadLeader;
	CBirdFlyer *pNext;
