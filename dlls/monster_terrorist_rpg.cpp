//HARD CODED FILE:
//Purpose:
//Create a new CZ enemy whit RPG weapon
...
#define HGRUNT_NUM_HEADS				5 // how many grunt heads are there? 
#define HGRUNT_MINIMUM_HEADSHOT_DAMAGE	25 // must do at least this much damage in one shot to head to score a headshot kill
...
#define GUN_GROUP					2
#define GUN_MP5						0// not used
#define GUN_SHOTGUN					0// this 1
#define GUN_NONE					1 // this 2
...
LINK_ENTITY_TO_CLASS( monster_human_rpg, CTerror );
...
BOOL CTerror :: CheckRangeAttack1 ( float flDot, float flDist )
{
	if ( !HasConditions( bits_COND_ENEMY_OCCLUDED ) && flDist <= 2048 && flDot >= 0.5 && NoFriendlyFire() )
	{
		...
		if ( !m_hEnemy->IsPlayer() && flDist <= 128 ) //SP - que sea el doble así no se revientan
		{
...
//	CBaseEntity *pRocket = CBaseEntity::Create( "SPA_rocket", vecShootOrigin, UTIL_VecToAngles( vecShootDir ), edict() );
...
void CTerror :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	switch( pEvent->event )
	{
		...
		case HGRUNT_AE_RELOAD:
			EMIT_SOUND( ENT(pev), CHAN_WEAPON, "humanrpg/t_reload1.wav", 1, ATTN_NORM );
	...
	//		CGrenade::ShootTimed( pev, pev->origin + gpGlobals->v_forward * 17 - gpGlobals->v_right * 27 + gpGlobals->v_up * 6, g_vecZero, 3 );
		...
		case HGRUNT_AE_BURST1:
		{
			if ( FBitSet( pev->weapons, HGRUNT_9MMAR ))
			{
				...
				if ( RANDOM_LONG(0,1) )
				{
					EMIT_SOUND( ENT(pev), CHAN_WEAPON, "humanrpg/t_mgun1.wav", 1, ATTN_NORM );
				}
				else
				{
					EMIT_SOUND( ENT(pev), CHAN_WEAPON, "humanrpg/t_mgun2.wav", 1, ATTN_NORM );
				}
			}
			else
			{
				...
				EMIT_SOUND(ENT(pev), CHAN_WEAPON, "humanrpg/rpg_fire-1.wav", 1, ATTN_NORM );
			}
...
//	SET_MODEL( ENT(pev),"models/human_rpg.mdl");
...
//		pev->weapons = HGRUNT_9MMAR | HGRUNT_HANDGRENADE; //SP FIX: Q sea el SHOWGUN
	...
	if (FBitSet( pev->weapons, HGRUNT_SHOTGUN ))
	{
		...
		m_cClipSize		= 1; //SP FIX: Fire and reload ...
	}
	else
	{
	...
	{		
		...
		switch (RANDOM_LONG(0,4))
		{
			...
			case 4: SetBodygroup( HEAD_GROUP, HEAD_OTHER); break;
		}
	}
...
//	PRECACHE_MODEL("models/human_rpg.mdl");

	PRECACHE_SOUND( "humanrpg/t_mgun1.wav" );
	PRECACHE_SOUND( "humanrpg/t_mgun2.wav" );
	
	PRECACHE_SOUND( "humanrpg/t_die1.wav" );
	PRECACHE_SOUND( "humanrpg/t_die2.wav" );
	PRECACHE_SOUND( "humanrpg/t_die3.wav" );

	PRECACHE_SOUND( "humanrpg/t_pain1.wav" );
	PRECACHE_SOUND( "humanrpg/t_pain2.wav" );
	PRECACHE_SOUND( "humanrpg/t_pain3.wav" );
	PRECACHE_SOUND( "humanrpg/t_pain4.wav" );
	PRECACHE_SOUND( "humanrpg/t_pain5.wav" );

	PRECACHE_SOUND( "humanrpg/t_reload1.wav" );

	PRECACHE_SOUND( "weapons/glauncher.wav" );

	PRECACHE_SOUND( "humanrpg/rpg_fire-1.wav" );
...
void CTerror :: PainSound ( void )
{
	if ( gpGlobals->time > m_flNextPainTime )
	{
		...
		{
		case 0:	
			EMIT_SOUND( ENT(pev), CHAN_VOICE, "humanrpg/t_pain3.wav", 1, ATTN_NORM );	
			break;
		case 1:
			EMIT_SOUND( ENT(pev), CHAN_VOICE, "humanrpg/t_pain4.wav", 1, ATTN_NORM );	
			break;
		case 2:
			EMIT_SOUND( ENT(pev), CHAN_VOICE, "humanrpg/t_pain5.wav", 1, ATTN_NORM );	
			break;
		case 3:
			EMIT_SOUND( ENT(pev), CHAN_VOICE, "humanrpg/t_pain1.wav", 1, ATTN_NORM );	
			break;
		case 4:
			EMIT_SOUND( ENT(pev), CHAN_VOICE, "humanrpg/t_pain2.wav", 1, ATTN_NORM );	
			break;
		}
...
void CTerror :: DeathSound ( void )
{
	...
	{
	case 0:	
		EMIT_SOUND( ENT(pev), CHAN_VOICE, "humanrpg/t_die1.wav", 1, ATTN_IDLE );	
		break;
	case 1:
		EMIT_SOUND( ENT(pev), CHAN_VOICE, "humanrpg/t_die2.wav", 1, ATTN_IDLE );	
		break;
	case 2:
		EMIT_SOUND( ENT(pev), CHAN_VOICE, "humanrpg/t_die3.wav", 1, ATTN_IDLE );	
		break;
	}
}
...
void CTerror :: SetActivity ( Activity NewActivity )
{
	...
	case ACT_RANGE_ATTACK1:
		...
		if (FBitSet( pev->weapons, HGRUNT_9MMAR))
		{
			...
			{
				// get crouching shoot
				iSequence = LookupSequence( "LAW_fire" ); //crouching_mp5
			}
		}
		else
		{
			...
			{
				// get aimable sequence
				iSequence = LookupSequence( "LAW_fire" ); //standing_shotgun
			}
			else
			{
				// get crouching shoot
				iSequence = LookupSequence( "LAW_fire" ); //crouching_shotgun
			}
		}
		break;
	case ACT_RANGE_ATTACK2:
...
//=========================================================
// CTerrorRepel - when triggered, spawns a monster_human_rpg
...
LINK_ENTITY_TO_CLASS( monster_human_rpg_repel, CTerrorRepel );
...
{
	UTIL_PrecacheOther( "monster_human_rpg" );
...
void CTerrorRepel::RepelUse ( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	*/
	...
	CBaseEntity *pEntity = Create( "monster_human_rpg", pev->origin, pev->angles );
