...
void CApache :: Killed( entvars_t *pevAttacker, int iGib )
{
	...
	if(IsCombineHelicopter)
	{
		STOP_SOUND( ENT(pev), CHAN_STATIC, "combinehelicopter/rotor1.wav" );
	//	STOP_SOUND( ENT(pev), CHAN_BODY, "apache/ap_whine1.wav" );
	}
...
/*
			...
			pSprite->Expand( RANDOM_FLOAT( 5, 10 ), RANDOM_FLOAT( 18.0, 26.0 )  );//expand
			pSprite->pev->frame = 0;
...
*/
		/*
			CSprite *pSprite = CSprite::SpriteCreate( "sprites/exp_end.spr", vecSpot, TRUE );
			pSprite->AnimateAndDie( RANDOM_FLOAT( 8.0, 12.0 ) );//framerate
			pSprite->SetTransparency( kRenderTransAlpha, 50, 50, 50, 255, kRenderFxNone );
			pSprite->SetScale( 5.0 );
			pSprite->Expand( RANDOM_FLOAT( 5, 10 ), RANDOM_FLOAT( 18.0, 26.0 )  );//expand
...
void CApache :: HuntThink( void )
{
	...
	UTIL_TraceLine ( vecEndPos, vecEndPos + Vector ( 0, 0, -768 ),  ignore_monsters, ENT(pev), &tr);//1024??

	if (tr.flFraction != 1.0) // != 1.0)// Hemos tocado algo
	{
//		ALERT ( at_console, "I touch something!  %f\n", tr.flFraction );

		if (tr.flFraction <= 0.3)
		{/*
			MESSAGE_BEGIN(MSG_ALL, gmsgParticles);
				WRITE_SHORT(0);
				WRITE_BYTE(0);
...
/*
			...
			pSprite->SetTransparency( kRenderTransAlpha, 222, 222, 222, 150, kRenderFxNone );
			pSprite->SetScale( 5 );//5.0
			pSprite->Expand( 10 , RANDOM_FLOAT( 30.0, 50.0 )  );//25
			pSprite->pev->frame = 0;

			pSprite->pev->avelocity.x = RANDOM_FLOAT( -150, 150 );//rotar?
			pSprite->pev->avelocity.y = RANDOM_FLOAT( -150, 150 );
			pSprite->pev->avelocity.z = RANDOM_FLOAT( -150, 150 );

			pSprite->pev->velocity.z = RANDOM_FLOAT( 15, 20 );//sppeed
			pSprite->pev->velocity.x = RANDOM_FLOAT( -300, 300 );//50
			pSprite->pev->velocity.y = RANDOM_FLOAT( -300, 300 );
			*/
		}
		else
		{/*
...
/*			CSprite *pSprite2 = CSprite::SpriteCreate( "sprites/exp_end.spr", tr.vecEndPos, TRUE );
				
			pSprite2->SetTransparency( kRenderTransAlpha, 200, 200, 200, 111, kRenderFxNone );
			pSprite2->SetScale( 5.0 );//5.0
			pSprite2->Expand( 10 , RANDOM_FLOAT( 20.0, 50.0 )  );//25
			pSprite2->pev->frame = 0;

			pSprite2->pev->avelocity.x = RANDOM_FLOAT( -150, 150 );//rotar?
			pSprite2->pev->avelocity.y = RANDOM_FLOAT( -150, 150 );
			pSprite2->pev->avelocity.z = RANDOM_FLOAT( -150, 150 );

			pSprite2->pev->velocity.z = RANDOM_FLOAT( 5, 10 );//sppeed
			pSprite2->pev->velocity.x = RANDOM_FLOAT( -250, 250 );//50
			pSprite2->pev->velocity.y = RANDOM_FLOAT( -250, 250 );*/
		}
	}
/*
	...
	Vector VecSrc;
	VecSrc = pev->origin + Vector (0,0, 100);
/*
	UTIL_Sparks( VecSrc );

	UTIL_ParticleEffect ( VecSrc + gpGlobals->v_forward * 128, g_vecZero, 255, 5 );
	UTIL_ParticleEffect ( VecSrc + gpGlobals->v_right * 128, g_vecZero, 100, 5 );
	UTIL_ParticleEffect ( VecSrc + gpGlobals->v_up * 128, g_vecZero, 50, 5 );
...
*/
	...
	{	
		...
		if (!FClassnameIs( pEntidad->pev, "npc_combine_helicopter" ) )
		if (!FClassnameIs( pEntidad->pev, "monster_apache" ) )
		{
			pEntidad->TakeDamage( pev, pev, 50, DMG_CRUSH );//20 y 20?
	...
	//Sys - This don't work very well
/*
    while ((pEntidad = UTIL_FindEntityInSphere( pEntidad, pev->origin, 512 )) != NULL)
	{	
		//!!!BUGBUG: Shit! My movetype is FLY! I need to override my own classname, at least	
		if (!FClassnameIs( pEntidad->pev, "npc_combine_helicopter" ) )
		if (!FClassnameIs( pEntidad->pev, "monster_apache" ) )
		{
			if (pEntidad->pev->movetype	== MOVETYPE_FLY)
			{
				pEntidad->pev->velocity.x = RANDOM_FLOAT( -100, 100 );//-100
				pEntidad->pev->velocity.y = -100;
				pEntidad->pev->velocity.z = -100;
			}
		}
	}
*/
...
void CApache :: Flight( void )
{
	...
	if (m_iSoundState == 0)
	{
		if(IsCombineHelicopter)
		{
			EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, "combinehelicopter/rotor1.wav", 1.0, 0.3, 0, 110 );
			//EMIT_SOUND_DYN(ENT(pev), CHAN_BODY, "apache/ap_whine1.wav", 0.5, 0.2, 0, 110 );
		}
		else
		{
		...
		//	EMIT_FMOD_SOUND( entindex(), pev->origin, "apache/ap_rotor2.mp3", 200, 100 );//fo0z
		}
	...
	else
	{
		...
		if (pPlayer)
		{
			...
			if(IsCombineHelicopter)
			{
				EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, "combinehelicopter/rotor1.wav", 1.0, 0.3, SND_CHANGE_PITCH | SND_CHANGE_VOL, pitch);
			}
			else
			{
				EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, "apache/ap_rotor2.wav", 1.0, 0.3, SND_CHANGE_PITCH | SND_CHANGE_VOL, pitch);
				EMIT_SOUND_DYN(ENT(pev), CHAN_BODY, "apache/ap_whine1.wav", 1.0, 0.2, SND_CHANGE_PITCH | SND_CHANGE_VOL, pitch);
			}
		}
...
BOOL CApache :: FireGun( )
{
	...
	if (DotProduct( vecGun, vecTarget ) > 0.98)
	{
		...
		if(IsCombineHelicopter)
		{
			FireIRGUNBullets( 1, posGun, vecGun, VECTOR_CONE_1DEGREES, 8192, BULLET_PLAYER_AK74, 1 );//4 12mm
			UTIL_Sparks( posGun );
			EMIT_SOUND(ENT(pev), CHAN_WEAPON, "combinehelicopter/fire1.wav", 1, 0.3);
		}
		else
		{
...
void CApache :: ShowDamage( void )
{
	if (m_iDoSmokePuff > 0 || RANDOM_LONG(0,99) > pev->health)
	{
		...
		pSprite->AnimateAndDie( RANDOM_FLOAT( 8.0, 12.0 ) );//framerate
		pSprite->SetTransparency( kRenderTransAlpha, 50, 50, 50, 255, kRenderFxNone );
		pSprite->SetScale(  RANDOM_LONG(3,6) );
		pSprite->Expand( 5, RANDOM_FLOAT( 98.0, 126.0 )  );//expand
		pSprite->pev->frame = 0;
		...
		pSprite->pev->velocity.z = RANDOM_FLOAT( -100, -300 );//abajo
...
int CApache :: TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType )
{
	...
	if (bitsDamageType & DMG_BLAST)
	{
		...
		if(IsCombineHelicopter)
		{
			EMIT_SOUND( ENT(pev), CHAN_VOICE, "combinehelicopter/damage1.wav", 1, 0.5 );
		}
	}
...
void CApacheHVR :: IgniteThink( void  )
{
	...
	pSprite->SetScale( 5.0 );
	pSprite->Expand( 5 , RANDOM_FLOAT( 80.0, 100.0 )  );//expand MENOS
	pSprite->pev->frame = 0;
...
///////////////////////////////////////////////////////////////
class CcombineHelicopter: public CApache
{
public:
	void Spawn( );
	void Precache(void);
...
LINK_ENTITY_TO_CLASS( npc_combine_helicopter, CcombineHelicopter );

void CcombineHelicopter :: Spawn( void )
{
	Precache( );
	// motor
	pev->movetype = MOVETYPE_FLY;
	...
	SET_MODEL(ENT(pev), "models/hl2/combine_helicopter.mdl");

	IsCombineHelicopter = TRUE;

	CApache :: Spawn(  );
}


void CcombineHelicopter::Precache( void )
{
	PRECACHE_MODEL("models/hl2/combine_helicopter.mdl");

	PRECACHE_SOUND("combinehelicopter/rotor1.wav");
	PRECACHE_SOUND("combinehelicopter/alert1.wav");
	PRECACHE_SOUND("combinehelicopter/damage1.wav");
	PRECACHE_SOUND("combinehelicopter/fire1.wav");
