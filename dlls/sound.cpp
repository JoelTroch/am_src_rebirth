...
NONE (Yet... I will use Fmod 3D Sound and this file will be obsolete)
...
#define CSENTENCEG_MAX 999					// max number of sentence groups
...
//	byte *pMemFile = g_engfuncs.pfnLoadFileForMe( "scripts/sentences.txt", &fileSize );
	...
	while ( memfgets(pMemFile, fileSize, filePos, buffer, 511) != NULL )
	{
		...
		{
			ALERT (at_error, "Demasiadas sentencias en sentences.txt!\n");
			break;
		}
		...
		if ( strlen( pString ) >= CBSENTENCENAME_MAX )
			ALERT( at_warning, "Sentencia %s es mas grande que %d letras\n", pString, CBSENTENCENAME_MAX-1 );
		...
		if (strcmp(szgroup, &(buffer[i])))
		{
			...
			{
				ALERT (at_error, "Demasiados grupos de sentencias en sentences.txt!\n");
				break;
			}
...
void EMIT_SOUND_DYN(edict_t *entity, int channel, const char *sample, float volume, float attenuation, int flags, int pitch)
{
	if (sample && *sample == '!')
	{
		...
		if (SENTENCEG_Lookup(sample, name) >= 0)
		{
			...
			EMIT_SOUND_DYN2(entity, channel, name, volume, attenuation, flags, 50);
			else
			EMIT_SOUND_DYN2(entity, channel, name, volume, attenuation, flags, pitch);
		}
		else
		{
			ALERT( at_aiconsole, "Imposible de encontrar %s dentro de sentences.txt\n", sample );
		}
	}
	else
	{
		if ( CVAR_GET_FLOAT("slowmo" ) != 0 )			
		EMIT_SOUND_DYN2(entity, channel, sample, volume, attenuation, flags, 50);
		else
...
//	if (pPlayer)
//	UTIL_ShowMessage( STRING(sample), pPlayer );//strings code
}
...
// fo0z fmod play sound
/*
extern int gmsgFmodPlaySound;

void EMIT_FMOD_SOUND( int entindex, Vector origin, const char *sample, int volume, int pitch)
{
	MESSAGE_BEGIN( MSG_ALL, gmsgFmodPlaySound, NULL );
...
//	pMemFile = g_engfuncs.pfnLoadFileForMe( "scripts/materials.txt", &fileSize );
	//this dont work for some reason
...
/*
	switch (chTextureType)//common/null.wav
	{
	default://don't send sounds
	...
	default://don't send sounds
	case CHAR_TEX_CONCRETE: fvol = 0.9;	fvolbar = 0.6;
		rgsz[0] = "common/null.wav";
		rgsz[1] = "common/null.wav";
		cnt = 2;
		break;
	case CHAR_TEX_METAL: fvol = 0.9; fvolbar = 0.3;
		rgsz[0] = "common/null.wav";
	...
	{//movido al trace de firebullets (player, trough walls, monsters, etc)
	}
	...
	if (iBulletType == BULLET_PLAYER_CROWBAR)
	{
	// crowbar already makes this sound
	}
	else
	{
		if ( RANDOM_LONG( 0, 1 ) == 0 )
		{
			...
			{
				case 0:UTIL_EmitAmbientSound(ENT(0), ptr->vecEndPos, "weapons/whizz1.wav", 1.5, fattn, 0, 96 + RANDOM_LONG(0,0xf));break;
				case 1:UTIL_EmitAmbientSound(ENT(0), ptr->vecEndPos, "weapons/whizz2.wav", 1.5, fattn, 0, 96 + RANDOM_LONG(0,0xf));break;
				case 2:UTIL_EmitAmbientSound(ENT(0), ptr->vecEndPos, "weapons/whizz3.wav", 1.5, fattn, 0, 96 + RANDOM_LONG(0,0xf));break;
				case 3:UTIL_EmitAmbientSound(ENT(0), ptr->vecEndPos, "weapons/whizz4.wav", 1.5, fattn, 0, 96 + RANDOM_LONG(0,0xf));break;
				case 4:UTIL_EmitAmbientSound(ENT(0), ptr->vecEndPos, "weapons/whizz5.wav", 1.5, fattn, 0, 96 + RANDOM_LONG(0,0xf));break;
			}
		}
	}
