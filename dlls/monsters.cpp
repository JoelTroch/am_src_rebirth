...
+monster_target -by LRC-
+prop_fire (a hard coded fire -please developers... don't kill me for that I just was testing!!-
...
//SysOp: Esto de 8 determina el paso a traves de objetos, escaleras?
...
TYPEDESCRIPTION	CBaseMonster::m_SaveData[] = 
{
	...
	DEFINE_FIELD( CBaseMonster, m_iPlayerReact, FIELD_INTEGER ),//sp

	DEFINE_FIELD( CBaseMonster, StartBurning, FIELD_BOOLEAN ),
...
//SysOp: Esto de 16 determina el paso a traves de objetos, escaleras?
...
//		REMOVE_ENTITY(ENT(pev));
	...
	//Sys- the mosnter see at 2048 units, but he wont shoot unless it's a 1024 units closer
...
void CBaseMonster :: StartMonster ( void )
{
	...
	if ( pev->movetype != MOVETYPE_FLY && !FBitSet( pev->spawnflags, SF_MONSTER_FALL_TO_GROUND ) )
	{
		...
		{
			ALERT(at_aiconsole, "Monster %s stuck in wall--level design error \n", STRING(pev->classname));
...
void CBaseMonster :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case SCRIPT_EVENT_NOT_DEAD:
		...
		break;
	//Sys: Mmmm... the monster doens't usually play another sounds except footstep sounds... Make a bit of dust
	//at the origin
...
BOOL CBaseMonster :: FindLateralCover ( const Vector &vecThreat, const Vector &vecViewOffset )
{
	...
	for ( i = 0 ; i < COVER_CHECKS ; i++ )
	{
		...
		if (tr.flFraction != 1.0)
		{
			...
			{
				if ( MoveToLocation( ACT_STRAFE_LEFT, 0, vecLeftTest ) )//sys test
				{
					return TRUE;
				}
			}
		...
		if ( tr.flFraction != 1.0 )
		{
			...
			{
				if ( MoveToLocation( ACT_STRAFE_RIGHT, 0, vecRightTest ) )//ACT_RUN
				{
					return TRUE;
				}
			}
...
void CBaseMonster :: MonsterInitDead( void )
{
	...
	pev->movetype		= MOVETYPE_NONE;//MOVETYPE_TOSS;// so he'll fall to ground
...
//Sys: Well... this is a really hard coded Fire... just look at that... :(
#define SF_MONSTERFIRED_OFF 1
class CMonsterFired : public CBaseMonster//BaseEntity//monster?
{
public:
	void Spawn( void );
	void Precache( void );
...
LINK_ENTITY_TO_CLASS( prop_fire, CMonsterFired );//prop_fire?

void CMonsterFired :: Spawn ( void )
{
...
//	pev->solid		= SOLID_BBOX;
	...
	MonsterInit();//init RUNAI and add damage later...
		
	if (pev->spawnflags & SF_MONSTERFIRED_OFF)
	{
		//perhaps add code later
	}
	else
	{
		TakeDamage( pev, pev, 1, DMG_SPECIALBURN);
	}
}

void CMonsterFired::Precache( void )
{
}


int	CMonsterFired :: Classify ( void )
{
	return CLASS_NONE;
}

void CMonsterFired::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	if (ShouldToggle( useType ))
	{
		TakeDamage( pev, pev, 1, DMG_SPECIALBURN);//burn always
	}
}