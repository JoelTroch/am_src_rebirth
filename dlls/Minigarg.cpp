//HARD CODE FILE. I need to make the gargantua's class to PUBLIC, and spawn this new entity from CGargantua...
//I really was a bad coder (well I didn't know, well, well... probabily i'm a bad coder now)
...
const float GARG_ATTACKDIST = 40.0;
...
#define GARG_DAMAGE					(DMG_ENERGYBEAM|DMG_CRUSH|DMG_MORTAR|DMG_BLAST|DMG_BULLET|DMG_CLUB)
...
#define garg_stomp_mini_SPRITE_NAME		"sprites/gargeye1.spr"
#define garg_stomp_mini_BUZZ_SOUND		"weapons/mine_charge.wav"
//#define GARG_FLAME_LENGTH			330
#define GARG_FLAME_LENGTH			150
...
int gStompSpriteMini = 0, gGargGibModelMini = 0;
void SpawnExplosionMini( Vector center, float randomRange, float time, int magnitude );

class CSmokerMini;

// Spiral Effect
class CSpiralMini : public CBaseEntity
{
public:
	void Spawn( void );
	void Think( void );
	int ObjectCaps( void ) { return FCAP_DONT_SAVE; }
	static CSpiralMini *Create( const Vector &origin, float height, float radius, float duration );
};
LINK_ENTITY_TO_CLASS( streak_spiral_mini, CSpiralMini );


class CStompMini : public CBaseEntity
{
public:
	void Spawn( void );
	void Think( void );
	static CStompMini *StompCreate( const Vector &origin, const Vector &end, float speed );
...
LINK_ENTITY_TO_CLASS( garg_stomp_mini, CStompMini );
CStompMini *CStompMini::StompCreate( const Vector &origin, const Vector &end, float speed )
{
	CStompMini *pStomp = GetClassPtr( (CStompMini *)NULL );
...
void CStompMini::Spawn( void )
{
	//SetNextThink( 0 );
	pev->nextthink = 0;
	pev->classname = MAKE_STRING("garg_stomp_mini");
	pev->dmgtime = gpGlobals->time;

	pev->framerate = 30;
	pev->model = MAKE_STRING(garg_stomp_mini_SPRITE_NAME);
	pev->rendermode = kRenderTransTexture;
	pev->renderamt = 0;
	EMIT_SOUND_DYN( edict(), CHAN_BODY, garg_stomp_mini_BUZZ_SOUND, 1, ATTN_NORM, 0, PITCH_NORM * 0.55);
}


#define	STOMP_INTERVAL		0.025

void CStompMini::Think( void )
{
	TraceResult tr;
...
//	SetNextThink( 0.1 );
	...
	while ( gpGlobals->time - pev->dmgtime > STOMP_INTERVAL )
	{
		...
		for ( int i = 0; i < 2; i++ )
		{
			CSprite *pSprite = CSprite::SpriteCreate( garg_stomp_mini_SPRITE_NAME, pev->origin, TRUE );
			if ( pSprite )
			{
		...
		if ( pev->scale <= 0 )
		{
			// Life has run out
			UTIL_Remove(this);
			STOP_SOUND( edict(), CHAN_BODY, garg_stomp_mini_BUZZ_SOUND );
		}
...
void StreakSplashMini( const Vector &origin, const Vector &direction, int color, int count, int speed, int velocityRange )
{
...
class CMiniGarg : public CBaseMonster
{
public:
		void DeathSound( void );//sys
	void Spawn( void );
	void Precache( void );
...
LINK_ENTITY_TO_CLASS( monster_mini_gargantua, CMiniGarg );

TYPEDESCRIPTION	CMiniGarg::m_SaveData[] = 
{
	DEFINE_FIELD( CMiniGarg, m_pEyeGlow, FIELD_CLASSPTR ),
	DEFINE_FIELD( CMiniGarg, m_eyeBrightness, FIELD_INTEGER ),
	DEFINE_FIELD( CMiniGarg, m_seeTime, FIELD_TIME ),
	DEFINE_FIELD( CMiniGarg, m_flameTime, FIELD_TIME ),
	DEFINE_FIELD( CMiniGarg, m_streakTime, FIELD_TIME ),
	DEFINE_FIELD( CMiniGarg, m_painSoundTime, FIELD_TIME ),
	DEFINE_ARRAY( CMiniGarg, m_pFlame, FIELD_CLASSPTR, 4 ),
	DEFINE_FIELD( CMiniGarg, m_flameX, FIELD_FLOAT ),
	DEFINE_FIELD( CMiniGarg, m_flameY, FIELD_FLOAT ),
};

IMPLEMENT_SAVERESTORE( CMiniGarg, CBaseMonster );

const char *CMiniGarg::pAttackHitSounds[] = 
{
...
const char *CMiniGarg::pBeamAttackSounds[] = 
{
...
const char *CMiniGarg::pAttackMissSounds[] = 
{
	"zombie/claw_miss1.wav",
	"zombie/claw_miss2.wav",
};

const char *CMiniGarg::pRicSounds[] = 
{
#if 0
	"weapons/ric1.wav",
	"weapons/ric2.wav",
...
const char *CMiniGarg::pFootSounds[] = 
{
	"garg/gar_step1.wav",
	"garg/gar_step2.wav",
};
...
const char *CMiniGarg::pIdleSounds[] = 
{
	"garg/gar_idle1.wav",
	"garg/gar_idle2.wav",
	"garg/gar_idle3.wav",
...
const char *CMiniGarg::pAttackSounds[] = 
{
...
const char *CMiniGarg::pAlertSounds[] = 
{
	"garg/gar_alert1.wav",
	"garg/gar_alert2.wav",
	"garg/gar_alert3.wav",
...
const char *CMiniGarg::pPainSounds[] = 
{
	"garg/gar_pain1.wav",
	"garg/gar_pain2.wav",
	"garg/gar_pain3.wav",
...
const char *CMiniGarg::pStompSounds[] = 
{
	"garg/gar_stomp1.wav",
};

const char *CMiniGarg::pBreatheSounds[] = 
{
...
Schedule_t	slGargMiniFlame[] =
{
	{ 
		tlGargMiniFlame,
		ARRAYSIZE ( tlGargMiniFlame ), 
		0,
		0,
		"GargFlame"
	},
...
Schedule_t	slGargMiniSwipe[] =
{
	{ 
		tlGargMiniSwipe,
		ARRAYSIZE ( tlGargMiniSwipe ), 
...
DEFINE_CUSTOM_SCHEDULES( CMiniGarg )
{
	slGargMiniFlame,
	slGargMiniSwipe,
};

IMPLEMENT_CUSTOM_SCHEDULES( CMiniGarg, CBaseMonster );


void CMiniGarg::EyeOn( int level )
{
	m_eyeBrightness = level;	
}


void CMiniGarg::EyeOff( void )
{
	m_eyeBrightness = 0;
}


void CMiniGarg::EyeUpdate( void )
{
	if ( m_pEyeGlow )
	{
...
void CMiniGarg::StompAttack( void )
{
	TraceResult trace;
	...
	CStompMini::StompCreate( vecStart, trace.vecEndPos, 0 );
	UTIL_ScreenShake( pev->origin, 12.0, 50.0, 2.0, 500 );
...
void CMiniGarg :: FlameCreate( void )
{
	int			i;
...
void CMiniGarg :: FlameControls( float angleX, float angleY )
{
	if ( angleY < -180 )
		angleY += 360;
	else if ( angleY > 180 )
...
void CMiniGarg :: FlameUpdate( void )
{
	int				i;
	...
	for ( i = 0; i < 2; i++ )
	{
		if ( m_pFlame[i] )
		{
			...
			{
				StreakSplashMini( trace.vecEndPos, trace.vecPlaneNormal, 6, 20, 50, 400 );
				streaks = TRUE;
...
void CMiniGarg :: FlameDamage( Vector vecStart, Vector vecEnd, entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iClassIgnore, int bitsDamageType )
{
...
void CMiniGarg :: FlameDestroy( void )
{
	int i;
...
void CMiniGarg :: PrescheduleThink( void )
{
...
//=========================================================
void CMiniGarg :: SetYawSpeed ( void )
{
	int ys;
...
// DeathSound 
//=========================================================
void CMiniGarg :: DeathSound ( void )
{
	switch (RANDOM_LONG(0,1))
	{
		case 0:	EMIT_SOUND(ENT(pev), CHAN_BODY, "garg/gar_die1.wav", 1.0, ATTN_NORM);
		case 1:	EMIT_SOUND(ENT(pev), CHAN_BODY, "garg/gar_die2.wav", 1.0, ATTN_NORM);
	}
}
...
void CMiniGarg :: Spawn()
{
	...
	if (pev->model)
		...
		SET_MODEL(ENT(pev), "models/minigarg.mdl");
...
void CMiniGarg :: Precache()
{
	...
	if (pev->model)
		...
		PRECACHE_MODEL("models/minigarg.mdl");
	...
	gStompSpriteMini = PRECACHE_MODEL( garg_stomp_mini_SPRITE_NAME );
	gGargGibModelMini = PRECACHE_MODEL( GARG_GIB_MODEL );
	PRECACHE_SOUND( garg_stomp_mini_BUZZ_SOUND );
...
void CMiniGarg::TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType )
{
	ALERT( at_aiconsole, "CMiniGarg::TraceAttack\n");
...
int CMiniGarg::TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
//	ALERT( at_aiconsole, "CMiniGarg::TakeDamage\n");
...
void CMiniGarg::DeathEffect( void )
{
	int i;
	UTIL_MakeVectors(pev->angles);
	...
	CSpiralMini::Create( deathPos, (pev->absmax.z - pev->absmin.z) * 0.6, 125, 1.5 );

	Vector position = pev->origin;
	position.z += 32;
	for ( i = 0; i < 7; i+=2 )
	{
		SpawnExplosionMini( position, 70, (i * 0.3), 60 + (i*20) );
		position.z += 15;
	}

	CBaseEntity *pSmoker = CBaseEntity::Create( "env_smoker_mini", pev->origin, g_vecZero, NULL );
	pSmoker->pev->health = 1;	// 1 smoke balls
	pSmoker->pev->scale = 46;	// 4.6X normal size
	pSmoker->pev->dmg = 0;		// 0 radial distribution
//	pSmoker->nextthink( 2.5 );	// Start in 2.5 seconds
	pSmoker->pev->nextthink = 2.5;
}


void CMiniGarg::Killed( entvars_t *pevAttacker, int iGib )
{
	EyeOff();
...
// 
//=========================================================
BOOL CMiniGarg::CheckMeleeAttack1( float flDot, float flDist )
{
...
BOOL CMiniGarg::CheckMeleeAttack2( float flDot, float flDist )
{
//	ALERT(at_aiconsole, "CheckMelee(%f, %f)\n", flDot, flDist);
...
//
//=========================================================
BOOL CMiniGarg::CheckRangeAttack1( float flDot, float flDist )
{
...
//=========================================================
void CMiniGarg::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	switch( pEvent->event )
	{
	...
	case GARG_AE_LEFT_FOOT:
		UTIL_ScreenShake( pev->origin, 4.0, 1.0, 1.0, 300 );
...
// players head)
//=========================================================
CBaseEntity* CMiniGarg::GargantuaCheckTraceHullAttack(float flDist, int iDamage, int iDmgType)
{
	TraceResult tr;
...
Schedule_t *CMiniGarg::GetScheduleOfType( int Type )
{
	...
	switch( Type )
	{
		case SCHED_MELEE_ATTACK2:
			return slGargMiniFlame;
		case SCHED_MELEE_ATTACK1:
			return slGargMiniSwipe;
		break;
	}
...
void CMiniGarg::StartTask( Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
// RunTask
//=========================================================
void CMiniGarg::RunTask( Task_t *pTask )
{
	switch ( pTask->iTask )
	{
/*
	case TASK_DIE:
		if ( gpGlobals->time > m_flWaitFinished )
		{
			...
			StopAnimation();
			pev->nextthink = 0.15;
			SetThink(&CMiniGarg:: SUB_Remove );
			int i;
			int parts = MODEL_FRAMES( gGargGibModelMini );
			for ( i = 0; i < 10; i++ )
			{
			...
			MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, pev->origin );
				...
				// Model
				WRITE_SHORT( gGargGibModelMini );	//model id#
...
class CSmokerMini : public CBaseEntity
{
public:
	void Spawn( void );
	void Think( void );
...
LINK_ENTITY_TO_CLASS( env_smoker_mini, CSmokerMini );

void CSmokerMini::Spawn( void )
{
...
void CSmokerMini::Think( void )
{
	...
	//	SetNextThink( RANDOM_FLOAT(0.1, 0.2) );
		pev->nextthink = RANDOM_FLOAT(0.1, 0.2);
	else
		UTIL_Remove( this );
}


void CSpiralMini::Spawn( void )
{
	pev->movetype = MOVETYPE_NONE;
//	SetNextThink( 0 );
	pev->nextthink = 0;
...
CSpiralMini *CSpiralMini::Create( const Vector &origin, float height, float radius, float duration )
{
	if ( duration <= 0 )
		return NULL;

	CSpiralMini *pSpiral = GetClassPtr( (CSpiralMini *)NULL );
	pSpiral->Spawn();
...
void CSpiralMini::Think( void )
{
	...
	while ( time > SPIRAL_INTERVAL )
	{
		...
		StreakSplashMini( position, Vector(0,0,1), RANDOM_LONG(8,11), 20, RANDOM_LONG(50,150), 400 );
