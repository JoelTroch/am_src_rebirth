...
#define garg_stomp_e_SPRITE_NAME		"sprites/gargeye1.spr"
#define garg_stomp_e_BUZZ_SOUND		"weapons/mine_charge.wav"
#define GARG_FLAME_LENGTH			430//sys
...
int gStompSpriteE = 0, gGargGibModelE = 0;
void SpawnExplosionE( Vector center, float randomRange, float time, int magnitude );

class CSmokerE;

// Spiral Effect
class CSpiralE : public CBaseEntity
{
public:
	void Spawn( void );
	void Think( void );
	int ObjectCaps( void ) { return FCAP_DONT_SAVE; }
	static CSpiralE *Create( const Vector &origin, float height, float radius, float duration );
};
LINK_ENTITY_TO_CLASS( streak_spiral_e, CSpiralE );


class CStompE : public CBaseEntity
{
public:
	void Spawn( void );
	void Think( void );
	static CStompE *StompCreate( const Vector &origin, const Vector &end, float speed );
...
LINK_ENTITY_TO_CLASS( garg_stomp_e, CStompE );
CStompE *CStompE::StompCreate( const Vector &origin, const Vector &end, float speed )
{
	CStompE *pStomp = GetClassPtr( (CStompE *)NULL );
...
void CStompE::Spawn( void )
{
	pev->nextthink = gpGlobals->time;
	pev->classname = MAKE_STRING("garg_stomp_e");
	pev->dmgtime = gpGlobals->time;

	pev->framerate = 30;
	pev->model = MAKE_STRING(garg_stomp_e_SPRITE_NAME);
	pev->rendermode = kRenderTransTexture;
	pev->renderamt = 0;
	EMIT_SOUND_DYN( edict(), CHAN_BODY, garg_stomp_e_BUZZ_SOUND, 1, ATTN_NORM, 0, PITCH_NORM * 0.55);
}


#define	STOMP_INTERVAL		0.025

void CStompE::Think( void )
{
	TraceResult tr;
	...
	while ( gpGlobals->time - pev->dmgtime > STOMP_INTERVAL )
	{
		...
		for ( int i = 0; i < 2; i++ )
		{
			CSprite *pSprite = CSprite::SpriteCreate( garg_stomp_e_SPRITE_NAME, pev->origin, TRUE );
			if ( pSprite )
			{
		...
		if ( pev->scale <= 0 )
		{
			// Life has run out
			UTIL_Remove(this);
			STOP_SOUND( edict(), CHAN_BODY, garg_stomp_e_BUZZ_SOUND );
		}
...
void StreakSplashE( const Vector &origin, const Vector &direction, int color, int count, int speed, int velocityRange )
{
...
class CGargantuaE : public CBaseMonster
{
public:
	void Spawn( void );
	void Precache( void );
...
LINK_ENTITY_TO_CLASS( monster_elec_gargantua, CGargantuaE );

TYPEDESCRIPTION	CGargantuaE::m_SaveData[] = 
{
	DEFINE_FIELD( CGargantuaE, m_pEyeGlow, FIELD_CLASSPTR ),
	DEFINE_FIELD( CGargantuaE, m_eyeBrightness, FIELD_INTEGER ),
	DEFINE_FIELD( CGargantuaE, m_seeTime, FIELD_TIME ),
	DEFINE_FIELD( CGargantuaE, m_flameTime, FIELD_TIME ),
	DEFINE_FIELD( CGargantuaE, m_streakTime, FIELD_TIME ),
	DEFINE_FIELD( CGargantuaE, m_painSoundTime, FIELD_TIME ),
	DEFINE_ARRAY( CGargantuaE, m_pFlame, FIELD_CLASSPTR, 4 ),
	DEFINE_FIELD( CGargantuaE, m_flameX, FIELD_FLOAT ),
	DEFINE_FIELD( CGargantuaE, m_flameY, FIELD_FLOAT ),
};

IMPLEMENT_SAVERESTORE( CGargantuaE, CBaseMonster );

const char *CGargantuaE::pAttackHitSounds[] = 
{
...
const char *CGargantuaE::pBeamAttackSounds[] = 
{
...
const char *CGargantuaE::pAttackMissSounds[] = 
{
	"zombie/claw_miss1.wav",
	"zombie/claw_miss2.wav",
};

const char *CGargantuaE::pRicSounds[] = 
{
#if 0
	"weapons/ric1.wav",
	"weapons/ric2.wav",
...
const char *CGargantuaE::pFootSounds[] = 
{
	"garg/gar_step1.wav",
	"garg/gar_step2.wav",
};
...
const char *CGargantuaE::pIdleSounds[] = 
{
	"garg/gar_idle1.wav",
	"garg/gar_idle2.wav",
	"garg/gar_idle3.wav",
...
const char *CGargantuaE::pAttackSounds[] = 
{
...
const char *CGargantuaE::pAlertSounds[] = 
{
	"garg/gar_alert1.wav",
	"garg/gar_alert2.wav",
	"garg/gar_alert3.wav",
...
const char *CGargantuaE::pPainSounds[] = 
{
	"garg/gar_pain1.wav",
	"garg/gar_pain2.wav",
	"garg/gar_pain3.wav",
...
const char *CGargantuaE::pStompSounds[] = 
{
	"garg/gar_stomp1.wav",
};

const char *CGargantuaE::pBreatheSounds[] = 
{
...
Schedule_t	slGargEFlame[] =
{
	{ 
		tlGargEFlame,
		ARRAYSIZE ( tlGargEFlame ), 
		0,
		0,
		"GargFlame"
	},
...
Schedule_t	slGargESwipe[] =
{
	{ 
		tlGargESwipe,
		ARRAYSIZE ( tlGargESwipe ), 
...
DEFINE_CUSTOM_SCHEDULES( CGargantuaE )
{
	slGargEFlame,
	slGargESwipe,
};

IMPLEMENT_CUSTOM_SCHEDULES( CGargantuaE, CBaseMonster );


void CGargantuaE::EyeOn( int level )
{
	m_eyeBrightness = level;	
}


void CGargantuaE::EyeOff( void )
{
	m_eyeBrightness = 0;
}


void CGargantuaE::EyeUpdate( void )
{
	if ( m_pEyeGlow )
	{
...
void CGargantuaE::StompAttack( void )
{
	TraceResult trace;
	...
	CStompE::StompCreate( vecStart, trace.vecEndPos, 0 );
...
void CGargantuaE :: FlameCreate( void )
{
	int			i;
	...
	for ( i = 0; i < 4; i++ )
	{
		...
		if ( m_pFlame[i] )
		{
			...
			if ( i < 2 )
				m_pFlame[i]->SetColor( 20, 20, 200 );//sys
			else
				m_pFlame[i]->SetColor( 200, 200, 200 );

			m_pFlame[i]->SetBrightness( 220 );//sys
			m_pFlame[i]->SetFlags( BEAM_FSHADEIN );
			m_pFlame[i]->SetScrollRate( 60 );//sys
...
void CGargantuaE :: FlameControls( float angleX, float angleY )
{
	if ( angleY < -180 )
		angleY += 360;
	else if ( angleY > 180 )
...
void CGargantuaE :: FlameUpdate( void )
{
	int				i;
	...
	for ( i = 0; i < 2; i++ )
	{
		if ( m_pFlame[i] )
		{
			...
			{
				StreakSplashE( trace.vecEndPos, trace.vecPlaneNormal, 3, 20, 50, 400 );
				//sys
				streaks = TRUE;
...
void CGargantuaE :: FlameDamage( Vector vecStart, Vector vecEnd, entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int iClassIgnore, int bitsDamageType )
{
...
void CGargantuaE :: FlameDestroy( void )
{
	int i;
...
void CGargantuaE :: PrescheduleThink( void )
{
...
//=========================================================
void CGargantuaE :: SetYawSpeed ( void )
{
	int ys;
...
// Spawn
//=========================================================
void CGargantuaE :: Spawn()
{
	Precache( );
	if (pev->model)
		SET_MODEL(ENT(pev), STRING(pev->model)); //LRC
	else
	SET_MODEL(ENT(pev), "models/garg_elec.mdl");
...
//=========================================================
void CGargantuaE :: Precache()
{
	int i;

	PRECACHE_MODEL("models/garg_elec.mdl");
	...
	gStompSpriteE = PRECACHE_MODEL( garg_stomp_e_SPRITE_NAME );
	gGargGibModelE = PRECACHE_MODEL( GARG_GIB_MODEL );
	PRECACHE_SOUND( garg_stomp_e_BUZZ_SOUND );
...
void CGargantuaE::TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType )
{
//	ALERT( at_aiconsole, "CGargantuaE::TraceAttack\n");
	...
	if ( bitsDamageType == 0)
	{
		if ( pev->dmgtime != gpGlobals->time || (RANDOM_LONG(0,100) < 20) )
		{
			...
			UTIL_WhiteSparks( ptr->vecEndPos, ptr->vecPlaneNormal, 0, 10, 500, 500 );//chispas

			UTIL_WhiteSparks( ptr->vecEndPos, ptr->vecPlaneNormal, 9, 20, 5, 100 );//puntos
...
int CGargantuaE::TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
//	ALERT( at_aiconsole, "CGargantuaE::TakeDamage\n");
...
void CGargantuaE::DeathEffect( void )
{
	int i;
	UTIL_MakeVectors(pev->angles);
	...
	CSpiralE::Create( deathPos, (pev->absmax.z - pev->absmin.z) * 0.6, 125, 1.5 );

	Vector position = pev->origin;
	position.z += 32;
	for ( i = 0; i < 7; i+=2 )
	{
		SpawnExplosionE( position, 70, (i * 0.3), 60 + (i*20) );
		position.z += 15;
	}

	CBaseEntity *pSmoker = CBaseEntity::Create( "env_smoker_e", pev->origin, g_vecZero, NULL );
	...
	CBaseEntity *pEffect = Create( "test_effect", pev->origin, pev->angles );
	pEffect->Use( this, this, USE_ON, 1 );
}


void CGargantuaE::Killed( entvars_t *pevAttacker, int iGib )
{
	EyeOff();
...
// 
//=========================================================
BOOL CGargantuaE::CheckMeleeAttack1( float flDot, float flDist )
{
...
BOOL CGargantuaE::CheckMeleeAttack2( float flDot, float flDist )
{
//	ALERT(at_aiconsole, "CheckMelee(%f, %f)\n", flDot, flDist);
...
//
//=========================================================
BOOL CGargantuaE::CheckRangeAttack1( float flDot, float flDist )
{
...
//=========================================================
void CGargantuaE::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	switch( pEvent->event )
	{
...
// players head)
//=========================================================
CBaseEntity* CGargantuaE::GargantuaCheckTraceHullAttack(float flDist, int iDamage, int iDmgType)
{
	TraceResult tr;
...
Schedule_t *CGargantuaE::GetScheduleOfType( int Type )
{
...
void CGargantuaE::StartTask( Task_t *pTask )
{
	switch ( pTask->iTask )
	{
...
// RunTask
//=========================================================
void CGargantuaE::RunTask( Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_DIE:
		if ( gpGlobals->time > m_flWaitFinished )
		{
			...
			SetThink( SUB_Remove );
			int i;
			int parts = MODEL_FRAMES( gGargGibModelE );
			for ( i = 0; i < 10; i++ )
			{
			...
			MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, pev->origin );
				...
				// Model
				WRITE_SHORT( gGargGibModelE );	//model id#
...
class CSmokerE : public CBaseEntity
{
public:
	void Spawn( void );
	void Think( void );
...
LINK_ENTITY_TO_CLASS( env_smoker_e, CSmokerE );

void CSmokerE::Spawn( void )
{
...
CSpiralE *CSpiralE::Create( const Vector &origin, float height, float radius, float duration )
{
	if ( duration <= 0 )
		return NULL;

	CSpiralE *pSpiral = GetClassPtr( (CSpiralE *)NULL );
	pSpiral->Spawn();
...
void CSpiralE::Think( void )
{
	...
	while ( time > SPIRAL_INTERVAL )
	{
		...
		StreakSplashE( position, Vector(0,0,1), RANDOM_LONG(8,11), 20, RANDOM_LONG(50,150), 400 );
